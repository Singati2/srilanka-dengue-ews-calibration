...
## Feasibility flag
**A fully isolated, bit-exact rerun is NOT currently feasible**, because the **Python modeling-stack package versions were not preserved** (no `requirements.txt`/`environment.yml`/lock/`pyproject` is committed). A **best-effort, tolerance-based** rerun is feasible for most components under a freshly pinned environment, but exact reproduction of floating-point metrics/CIs cannot be guaranteed without the original package versions. The R `dlnm` comparator is fully pinned (R 4.6.0, dlnm 2.4.10, mgcv 1.9.4, tsModel 0.6-2) and is the most reproducible component.
...
- R: 4.6.0; dlnm 2.4.10; mgcv 1.9.4; tsModel 0.6-2 (verified — pin exactly).
