...
## Precision note
Full-precision test-inverted bounds are preserved in the machine-readable file `S11_srilanka_wild_cluster_bootstrap_t_results.csv` and are not printed in the manuscript or the SI text. Those bounds are numerical, single-implementation, Monte-Carlo-limited, and are not exact confidence limits; the manuscript-facing S11 table reports rounded intervals accordingly.
