...
Secondary evaluation restricted to calendar year 2022 (excluding 2020–2021), using the frozen recalibrated (post-Platt) predictions `colombia_model_predictions_h4_75pct_v1.csv` (sha256 `a938a138…`). No model refit, no recomputation; values transcribed from the finalized secondary-sensitivity outputs (`colombia_2022_only_v2` report; underlying `colombia_2022_only_results.csv` / `colombia_2022_only_contrasts.csv`, v1 run retained unchanged). Not interpreted as pandemic-free or post-pandemic validation.
