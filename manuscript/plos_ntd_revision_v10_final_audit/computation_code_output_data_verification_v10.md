# Computation / code / output / dataset verification — v10 (corrected)

## Scope and corrected verdict
**Scope statement.** No analytical pipeline, model-fitting, prediction, calibration, recalibration, bootstrap or sensitivity-analysis script was executed. No model-derived metric or confidence interval was recomputed. Manuscript builds, file inspection, hashes, row/count checks, word counts and simple deterministic arithmetic were performed. This is a read-only traceability and code-inspection audit; it does not assert that "no computation occurred", that "no script of any kind was run", or that "everything was independently verified".
