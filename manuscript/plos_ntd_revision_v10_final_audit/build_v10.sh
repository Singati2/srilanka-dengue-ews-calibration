#!/usr/bin/env bash
# Fail-fast build for PLOS NTD revision v10. Produces TWO review PDFs:
#   1) dengue_ews_v10_internal_author_review.pdf   (line numbers OFF; author standing preference)
#   2) dengue_ews_v10_line_numbered_review.pdf      (continuous line numbers ON)
...
set -euo pipefail
cd "$(dirname "$0")"
TEX="dengue_ews_plos_ntd_v10.tex"
SIDIR="supporting_information"

fail() { echo "FAIL: $1"; exit 1; }

# --- SI-file-existence guard: every S-item the manuscript lists must exist as a real file ---
for s in S1_STROBE_checklist_v10 S2_S3_TRIPOD_AI_PROBAST_v10 S4_analysis_plan_chronology_v10 \
         S5_country_model_specification_v10 S6_bootstrap_protocol_v10 S7_srilanka_sensitivity_grid_v10 \
         S8_srilanka_targeted_value_v10 S9_colombia_threshold_horizon_v10 S10_reproducibility_inventory_v10; do
...
build "dengue_ews_v10_internal_author_review" ""
# 2) line-numbered review
build "dengue_ews_v10_line_numbered_review" "\\def\\linenumberedcopy{}"

echo "BUILD OK: dengue_ews_v10_internal_author_review.pdf and dengue_ews_v10_line_numbered_review.pdf"
