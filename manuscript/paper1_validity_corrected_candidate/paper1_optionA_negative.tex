...
We conducted a retrospective decision-evaluation in Sri Lanka (primary, RDHS-week, 2018--2025) and Colombia (external framework reanalysis, 2020--2022), comparing a recent-case surveillance baseline (M1) with climate-only, structured-climate, and hybrid models by discrimination, calibration, recalibration, and decision-curve net benefit at a four-week horizon and a reference threshold of $p^*=0.30$. Recent surveillance was a demanding benchmark: climate-only models added no decision value, and the pre-specified primary hybrid contrast in Sri Lanka was null---$\Delta$NB(M4$-$M1) at $p^*=0.30$ was $-0.008$ (95\% CI $-0.028$ to $+0.013$)---with no model reliably exceeding recent surveillance (M1) across the evaluated label--horizon cells. Apparent positive signals arose only from structurally unmatched or pandemic-window comparisons. In Colombia, the full hybrid exceeded M1 by $\Delta$NB(M5$-$M1) $+0.0188$ (95\% CI $+0.0117$ to $+0.0260$), but M5 adds seasonal terms and 32 department fixed effects that M1 lacks, so this compound contrast is not a climate-specific effect. When climate was isolated against a baseline matched for that structure, its increment fell to $+0.0078$ (95\% CI $+0.0039$ to $+0.0119$)---a lower bound below the study's own $+0.005$ reference level---and an order-averaged (Shapley) attribution assigned climate roughly half of the compound difference (climate $\approx+0.0100$; non-climate structure $\approx+0.0087$). Climate's marginal net-benefit value declined once non-climate structure was in the baseline (decline $+0.0044$, 95\% CI $+0.0008$ to $+0.0081$). The Colombia increment did not grow with lead time (flat across 1--12 weeks) and derived from the 2020--2022 pandemic window (2022-only $+0.0150$). The two settings used asymmetric climate implementations and were not a like-for-like replication, and no cross-country heterogeneity test was performed.

\noindent\textbf{Conclusions/Significance.}
Standard early-warning evaluations---benchmarking climate against climatology or a null model and judging by discrimination---can overstate the value of climate data for dengue alerting. When we benchmarked against recent surveillance, judged by calibration and decision-curve net benefit, and compared against a structurally matched baseline, climate's standalone decision value collapsed and its isolated incremental value did not clear a permissive self-chosen reference band. The contribution is therefore methodological rather than a positive climate finding: dengue alerting evaluations should benchmark against recent surveillance, match non-climate structure before attributing value to climate, and report calibration and threshold-averaged decision-curve net benefit rather than discrimination against weak comparators. Local evaluation remains necessary, and the modest Colombia signal---confined to a pandemic window, flat across lead times, and dependent on non-climate structure---should not be read as evidence of transportable climate value.
\end{abstract}
