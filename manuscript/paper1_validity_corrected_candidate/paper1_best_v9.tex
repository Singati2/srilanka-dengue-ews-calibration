...
\thispagestyle{fancy}
\begin{center}
{\Large \textbf{Standard evaluation overstates the incremental value of climate data for dengue alerting: a surveillance-benchmarked, matched-baseline decision-curve reanalysis in Sri Lanka and Colombia}}\\[1.0em]
...
We conducted a retrospective decision-evaluation in Sri Lanka (primary, RDHS-week, 2018--2025) and Colombia (a second, differently-constructed case study, 2020--2022), comparing a recent-case surveillance baseline (M1) with climate-only, structured-climate, and hybrid models by discrimination, calibration, recalibration, and decision-curve net benefit at a four-week horizon and a reference threshold of $p^*=0.30$. Recent surveillance was a demanding benchmark: climate-only models added no decision value, and the pre-specified primary hybrid contrast in Sri Lanka was null---$\Delta$NB(M4$-$M1) at $p^*=0.30$ was $-0.008$ (95\% CI $-0.028$ to $+0.013$)---with no model reliably exceeding recent surveillance (M1) across the evaluated label--horizon cells. Apparent positive signals arose only from structurally unmatched or pandemic-window comparisons. In Colombia, the full hybrid exceeded M1 by $\Delta$NB(M5$-$M1) $+0.0188$ (95\% CI $+0.0117$ to $+0.0260$), but M5 adds seasonal terms and 32 department fixed effects that M1 lacks, so this compound contrast is not a climate-specific effect. When climate was isolated against a baseline matched for that structure, its increment fell to $+0.0078$ (95\% CI $+0.0039$ to $+0.0119$)---small but distinguishable from zero, roughly half the compound difference (order-averaged climate value $+0.0100$, 95\% CI $+0.0060$ to $+0.0142$; non-climate structure $\approx+0.0087$). This residual increment was small and its robustness could not be established: it was unchanged under a distributed-lag nonlinear specification but decayed toward zero under realistic reporting delay (three-week matched estimate $+0.0049$, 95\% CI $+0.0001$ to $+0.0097$), and the one pre-pandemic check available was confounded by the 2016 dengue--Zika co-circulation year and the 2019 record epidemic; no clean post-pandemic evaluation was performed (2023, the obvious candidate, was itself a record-epidemic year). It did not appear in Sri Lanka and did not grow with lead time, and no cross-country heterogeneity test was performed; the two settings used differently constructed climate exposures (Sri Lanka included humidity, Colombia did not) and are not a like-for-like replication.

\noindent\textbf{Conclusions/Significance.}
Standard early-warning evaluations---benchmarking climate against climatology or a null model and judging by discrimination---can overstate the value of climate data for dengue alerting. When we benchmarked against recent surveillance, judged by calibration and decision-curve net benefit, and matched for non-climate structure, climate's standalone decision value was largely absent and the apparent hybrid advantage shrank by roughly half. A small climate increment (matched $+0.0078$) nonetheless remained, but its robustness was not established---it decayed toward zero under realistic reporting delay and its only pre-pandemic check was epidemic- and Zika-confounded, with the definitive post-pandemic evaluation still to be run---and it did not appear in Sri Lanka; it is therefore not evidence of a general, transportable climate benefit. The contribution is therefore primarily methodological: dengue alerting evaluations should benchmark against recent surveillance, match non-climate structure before attributing value to climate, and report calibration and threshold-averaged decision-curve net benefit rather than discrimination against weak comparators. Local, setting-specific evaluation remains necessary.
\end{abstract}
...
Dengue early-warning systems often use weather and climate data to anticipate weeks of elevated dengue activity, but health agencies already track recent dengue case counts, which provide strong short-term information. We asked whether adding climate information improves alert decisions beyond what recent cases already show. We evaluated models in Sri Lanka and Colombia against a recent-surveillance benchmark, using calibration and decision-curve net benefit as well as ranking accuracy. Recent surveillance was difficult to outperform, and climate-only models added little decision value. In Sri Lanka, the originally planned combined model had a slightly lower net-benefit point estimate than surveillance, with substantial uncertainty, while an expanded combined model showed only a small and uncertain increment. In Colombia, the full combined model improved decision performance over recent cases, but it also contained seasonal and department-level terms absent from the recent-case model, so the original comparison did not isolate climate. After matching those non-climate components, the climate terms retained a smaller positive improvement; because this additional analysis was done after the fact and at a single decision threshold, it should be interpreted cautiously. That improvement did not increase across the evaluated lead times, while in Sri Lanka no combined model clearly beat recent surveillance across the evaluated label and horizon combinations. Because we did not formally compare the two settings, each result requires local interpretation. Our central message is about evaluation: comparisons that judge models against weak baselines, or that do not match this non-climate structure, can make climate data look more useful for alerting than it is. A fair test---against recent surveillance, matched for structure, and judged by calibrated decision value---found little standalone benefit from climate.
...
\bibitem{Leung2022} Leung XY, Islam RA, Adhami M, et al. A systematic review of dengue outbreak prediction models: current scenario and future directions. PLoS Negl Trop Dis. 2022. [volume/DOI to be confirmed]

\bibitem{ColonGonzalez2021} Col\'{o}n-Gonz\'{a}lez FJ, Bastos LS, Hofmann B, et al., Lowe R. Probabilistic seasonal dengue forecasting in Vietnam: a modelling study using superensembles. PLoS Med. 2021;18(3):e1003542. doi:\,10.1371/journal.pmed.1003542.
...
\textbf{S2 Checklist.} TRIPOD+AI reporting checklist; records that flexible graphical calibration curves were not generated.\\
