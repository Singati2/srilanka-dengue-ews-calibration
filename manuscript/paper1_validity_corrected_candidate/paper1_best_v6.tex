...
We stress-tested the residual \emph{matched} climate increment, $\Delta$NB(M5$-$M5$_{\text{no-climate}}$), against three alternatives, re-computing that same matched contrast in each case; the primary compound ($+0.0188$) and matched ($+0.0079$) estimates were reproduced exactly (to $<0.0001$) by the reimplementation before each test. (i) \emph{Pre-pandemic period.} On a single pre-specified pre-pandemic split (train $\le$2015, validation 2016--2017, test 2018--2019, with the 2018--2019 test data entering neither training nor recalibration), the matched increment was $+0.0199$ (95\% CI $+0.0012$ to $+0.0400$): the same sign as the full-period estimate, but larger and less precise. We therefore claim \emph{sign-stability}, not magnitude-robustness; and because the pre-pandemic period had high elevated-activity prevalence ($\approx0.50$), at $p^*=0.30$ alert-all is competitive and absolute magnitudes are not comparable across periods. (ii) \emph{Functional form.} Refitting the climate terms as a distributed-lag nonlinear cross-basis (natural-spline value $\times$ lag bases) in temperature and precipitation left the matched increment essentially unchanged, if slightly larger ($+0.0122$, 95\% CI $+0.0078$ to $+0.0160$); it is therefore not an artifact of the linear-lag form. Humidity was unavailable for Colombia and could not be added, so this test varies the functional form, not the variable set. (iii) \emph{Reporting delay.} Emulating reporting delay by censoring the most recent case lags---which degrades the surveillance predictors but leaves the outcome label and the climate vintages at their finalized values---left the matched increment approximately stable across censoring depths ($+0.0079$ uncensored, $+0.0075$ [95\% CI $+0.0029$ to $+0.0133$] at two-week, $+0.0051$ at three-week). The \emph{compound} $\Delta$NB(M5$-$M1) grew over the same range ($+0.0188\to+0.0282$), but that reflects degradation of the recent-case baseline, not a gain specific to climate; the climate-specific (matched) increment did not grow. These are sensitivity analyses conducted for this reanalysis using a Python reimplementation of the frozen pipeline (which reproduced the primary estimate exactly); the reporting-delay analysis is a surveillance-side feature-censoring emulation, not true reporting-triangle vintaged data, and real triangles would also degrade the label and climate vintages. Together they indicate the matched climate increment is \emph{sign-stable} within Colombia across pre-pandemic exclusion, functional form, and surveillance-side reporting delay, while remaining small, of uncertain magnitude, and absent in Sri Lanka.
