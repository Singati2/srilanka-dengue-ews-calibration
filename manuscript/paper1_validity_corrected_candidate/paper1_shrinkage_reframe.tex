...
We conducted a retrospective decision-evaluation in Sri Lanka (primary, RDHS-week, 2018--2025) and Colombia (external framework reanalysis, 2020--2022), comparing a recent-case surveillance baseline (M1) with climate-only, structured-climate, and hybrid models by discrimination, calibration, recalibration, and decision-curve net benefit at a four-week horizon and a reference threshold of $p^*=0.30$. Recent surveillance was a demanding benchmark: climate-only models added no decision value, and the pre-specified primary hybrid contrast in Sri Lanka was null---$\Delta$NB(M4$-$M1) at $p^*=0.30$ was $-0.008$ (95\% CI $-0.028$ to $+0.013$)---with no model reliably exceeding recent surveillance (M1) across the evaluated label--horizon cells. Apparent positive signals arose only from structurally unmatched or pandemic-window comparisons. In Colombia, the full hybrid exceeded M1 by $\Delta$NB(M5$-$M1) $+0.0188$ (95\% CI $+0.0117$ to $+0.0260$), but M5 adds seasonal terms and 32 department fixed effects that M1 lacks, so this compound contrast is not a climate-specific effect. When climate was isolated against a baseline matched for that structure, the increment shrank to $+0.0078$ (95\% CI $+0.0039$ to $+0.0119$); it remained distinguishable from zero but was roughly half the compound difference (order-averaged climate value $+0.0100$, 95\% CI $+0.0060$ to $+0.0142$), a shrinkage of about 1.5--2.4-fold depending on attribution order. Two features argue against reading even this residual as transportable climate value. First, the increment arose in Colombia's \emph{poorer} climate model (linear lags, no humidity), whereas Sri Lanka's \emph{richer} distributed-lag nonlinear model with humidity gave the null---the opposite of what a genuine climate signal predicts. Second, the Colombia signal is confined to the 2020--2022 pandemic window (2022-only $+0.0150$) and does not grow with lead time, and the matched baseline's fixed effects absorb time-invariant but not pandemic-shock structure. The two settings used differently constructed climate exposures and are not a like-for-like replication, and no cross-country heterogeneity test was performed.

\noindent\textbf{Conclusions/Significance.}
Standard early-warning evaluations---benchmarking climate against climatology or a null model and judging by discrimination---can overstate the value of climate data for dengue alerting. Benchmarked against recent surveillance, judged by calibration and decision-curve net benefit, and matched for non-climate structure, the apparent climate increment shrank by roughly half; given its dependence on a single pandemic-era window and on the poorer of the two settings' climate models, it is not credible evidence of transportable climate value. Under finalized-count evaluation, recent surveillance was a demanding benchmark. The contribution is therefore methodological rather than a positive climate finding: dengue alerting evaluations should benchmark against recent surveillance, match non-climate structure before attributing value to climate, and report calibration and threshold-averaged decision-curve net benefit rather than discrimination against weak comparators. Local, setting-specific evaluation remains necessary.
\end{abstract}
