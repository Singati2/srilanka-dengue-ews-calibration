...
% =====================================================================
% PLOS NTD revision v16 (framing cleanup).
% PROVENANCE: Derived directly from the preserved revision v15 (final-verification)
% source. v16 is a narrowly scoped framing-only cleanup (no new numbers, no recomputation):
% between-setting wording states that no formal interaction/heterogeneity/equivalence
% analysis was performed and that the settings are complementary rather than a formal
% comparative experiment (no overlapping-CI or equivalence reasoning); the hybrid
% estimates are reported in estimate-and-uncertainty language without classifying them by
% whether intervals contain zero; the wild-cluster-bootstrap-t is described as a secondary,
% single-implementation sensitivity that did not independently validate the percentile
% intervals; a compact future-work paragraph is added; and an elevated-activity clause is
% added to the limitations. Frozen numbers are unchanged.
% No statistical/predictive analysis was rerun; numbers are reused verbatim from frozen
% committed reports and the audited v2 outputs. Administrative metadata action items are
% tracked in submission_blockers_v16.md (not in the manuscript body). Not for submission.
% =====================================================================
...
\begin{table}[htbp]
\centering
\caption{Colombia $h=4$ primary common-complete test performance (external framework replication). Net benefit at $p^*=0.30$; alert-all reference $\approx$0.107, alert-none $0$. Test prevalence was 0.375 for the common-complete set shared by M0--M5. Per-model values for all six ladder models (M0--M5) are shown for completeness; the reported primary Colombia hybrid contrast is M5$-$M1, and the M4$-$M1 contrast is reported in Supporting Information (S9 Table) rather than in the main text. Per-model mean predicted probabilities were not retained in the committed reproducibility record. Calibration-in-the-large, calibration slope, and Brier-score results are summarized in the text and Supporting Information.}
\label{tab:colombia}
\small
\begin{tabular}{llrr}
\toprule
...
The Sri Lanka and Colombia hybrid estimates are shown side by side in Table~\ref{tab:crosssetting}. In Sri Lanka, the M5$-$M1 net-benefit estimate was $+0.0081$, with a 95\% CI from $-0.0012$ to $+0.0181$. In Colombia, the M5$-$M1 estimate was $+0.0188$, with a 95\% CI from $+0.0117$ to $+0.0260$. No formal interaction, heterogeneity or equivalence analysis was performed. The setting-specific estimates differed in magnitude and precision, but this study does not establish whether the underlying effects differ between settings. The two settings should be interpreted as complementary rather than as a formal comparative experiment. Recent surveillance was a strong benchmark in both settings; climate-only models added limited decision value in both; and the hybrid increment was modest and varied in magnitude and precision across the evaluated settings.
...
The limited decision value of the climate-only models should not be read as climate being irrelevant to transmission. Recent case counts already summarize upstream processes, including climate effects, so over short-to-medium horizons a recent-case baseline is a demanding but appropriate comparator, and even a canonical distributed-lag nonlinear climate model did not exceed it at the primary threshold. The hybrid results are the key nuance. In Sri Lanka, the planned M4$-$M1 estimate was $-0.008$ (95\% CI $-0.028$ to $+0.013$), while the expanded M5$-$M1 sensitivity estimate was $+0.0081$ (95\% CI $-0.0012$ to $+0.0181$). In Colombia, the M5$-$M1 estimate was $+0.0188$ (95\% CI $+0.0117$ to $+0.0260$), and the secondary 2022-only estimate was $+0.0150$ (95\% percentile interval $+0.0067$ to $+0.0249$). Two author-authorized secondary sensitivities, included to support rather than replace the primary findings, were consistent with this interpretation: the additional sensitivities did not materially alter the study's interpretation, in that the 2022-only Colombia estimate was modestly smaller than the full-period estimate, and the Sri Lanka wild-cluster-bootstrap-t analysis, a secondary, single-implementation sensitivity, produced uncertainty intervals similar to the original percentile intervals but did not independently validate them (Supporting Information). The original percentile-bootstrap intervals remain the primary reported uncertainty, the Colombia 2020--2022 analysis remains the primary replication, and neither sensitivity was treated as a primary analysis. No formal cross-setting comparison was performed; the two settings should be interpreted as complementary rather than as a formal comparative experiment. Climate value may be conditional on setting, surveillance dynamics, and outcome and threshold definitions, and is most defensible as an incremental signal within a hybrid system that requires local evaluation. The magnitude of the Colombia increment was modest: at $p^*=0.30$ it corresponds to approximately 1.9 net true-positive equivalents (approximate interval 1.2 to 2.6) per 100 municipality-weeks, or, at a fixed number of true-positive detections, approximately 4.4 fewer false-alert equivalents (approximate interval 2.7 to 6.1) per 100 --- decision-analytic representations of the same net-benefit difference, not observed public-health outcomes.
