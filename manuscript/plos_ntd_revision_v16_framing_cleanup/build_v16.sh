#!/usr/bin/env bash
# Fail-fast build for PLOS NTD revision v16 (integrity cleanup). Produces THREE PDFs:
#   1) dengue_ews_v16_internal_author_review.pdf   (line numbers OFF)
#   2) dengue_ews_v16_line_numbered_review.pdf      (continuous line numbers ON)
#   3) dengue_ews_v16_line_numbered_review_grayscale.pdf  (grayscale of #2)
set -euo pipefail
cd "$(dirname "$0")"
TEX="dengue_ews_plos_ntd_v16.tex"
SIDIR="supporting_information"

fail() { echo "FAIL: $1"; exit 1; }

for s in S1_STROBE_checklist_v16 S2_S3_TRIPOD_AI_PROBAST_v16 S4_analysis_plan_chronology_v16 \
         S5_country_model_specification_v16 S6_bootstrap_protocol_v16 S7_srilanka_sensitivity_grid_v16 \
         S8_srilanka_targeted_value_v16 S9_colombia_threshold_horizon_v16 S10_reproducibility_inventory_v16 \
         S11_srilanka_wild_cluster_bootstrap_t_v16 S12_srilanka_wild_bootstrap_method_v16; do
...
build "dengue_ews_v16_internal_author_review" ""
build "dengue_ews_v16_line_numbered_review" "\\def\\linenumberedcopy{}"

GRAY="dengue_ews_v16_line_numbered_review_grayscale.pdf"
rm -f "$GRAY"
if command -v gs >/dev/null 2>&1; then
  gs -q -o "$GRAY" -sDEVICE=pdfwrite -sColorConversionStrategy=Gray \
     ...
     "dengue_ews_v16_line_numbered_review.pdf" >/dev/null 2>&1 || fail "grayscale (gs) conversion failed"
elif command -v pdftocairo >/dev/null 2>&1; then
  pdftocairo -pdf -gray "dengue_ews_v16_line_numbered_review.pdf" "$GRAY" >/dev/null 2>&1 || fail "grayscale (pdftocairo) conversion failed"
else
