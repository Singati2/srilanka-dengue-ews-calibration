...
- Colombia OpenDengue extract (provenance distinction):
  ...
  - Locally verified version: V1.3 (verified from the analyzed file).
  - Generic Figshare record DOI (cited in references): `10.6084/m9.figshare.24259573`.
  - Unresolved record-version reconciliation: a prior reference audit found the cited record DOI resolves to a V1.2 record; a version-specific DOI for V1.3 is not pinned (open item; see submission blockers).
  - Acquisition date: not preserved in the committed acquisition record (open item).
...
## Secondary-sensitivity artifacts (added in v12, retained in v13–v14; transcribed from audited v2 outputs, no recomputation)
