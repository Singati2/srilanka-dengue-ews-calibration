...
% =====================================================================
% PLOS NTD revision v12 (sensitivity integration).
% PROVENANCE: Derived directly from the preserved revision v10 (final-audit) source.
% v11 (manuscript-integrity) was not produced as a standalone revision; its outstanding
% integrity corrections are applied here in v12 alongside cautious integration of two
% author-authorized secondary sensitivity analyses (Colombia 2022-only; Sri Lanka
% wild-cluster-bootstrap-t), using only the finalized audited v2 sensitivity outputs.
% Numbers are reused verbatim from frozen committed reports and the audited v2 outputs
% (no recomputation; no model refit; no new bootstrap). Unresolved administrative
% metadata are tracked in submission_blockers (not in the manuscript body). Not for
% submission until the authors confirm those items.
% =====================================================================
...
Dengue early-warning systems often use weather and climate data to anticipate weeks of elevated dengue activity, and they are usually judged by how well they separate high-risk from low-risk weeks. Here, an alert week was one in which dengue activity exceeded a locally defined historical level, not an official outbreak declaration. Health agencies already track recent dengue case counts, which carry strong short-term information. We asked a practical question: does adding climate information improve alert decisions beyond what recent cases already tell them? We evaluated such models in Sri Lanka and Colombia against a recent-case surveillance benchmark, judging them by calibration and alert decisions, not only ranking accuracy. Recent surveillance was hard to beat, and climate alone added little. The originally planned combined model did not beat surveillance; a more elaborate combined model gave only a small, uncertain increment that varied across the two datasets and did not grow at longer lead times. We did not formally test whether the two settings differed, so each result needs local interpretation. Climate information looks more useful as a locally assessed complement to recent surveillance than as a standalone replacement, and warrants local evaluation.
...
In a secondary evaluation restricted to 2022, excluding observations from 2020--2021, the M5$-$M1 net-benefit estimate at $p^*=0.30$ was $+0.0150$ (95\% percentile interval $+0.0067$ to $+0.0249$), compared with $+0.0188$ in the full 2020--2022 test period. The 2022 subset contained 4{,}802 municipality-weeks from 274 municipalities (48 eligible four-week-ahead prediction weeks), with 2{,}202 elevated-activity events and prevalence 0.4586. Because prevalence was higher in 2022, alert-all was substantially more competitive (alert-all net benefit $\approx$0.227 in the 2022 subset, against $\approx$0.107 in the full test period), so absolute net-benefit levels in the subset are higher and are not directly comparable to the full-period levels; the within-subset M5$-$M1 contrast is the comparable quantity. This subset was not interpreted as pandemic-free or post-pandemic validation. The subset comprised 48 eligible prediction weeks rather than 52 because, under the four-week-ahead horizon, the four December-2022 prediction weeks have no observed outcome four weeks later within the assembled OpenDengue Admin-2 series (which ends in late November 2022), so they are not eligible; the planned-primary contrast in this subset, $\Delta\mathrm{NB}(\text{M4}-\text{M1})$, was $+0.0097$ (95\% percentile interval $+0.0048$ to $+0.0150$). Full per-model metrics and protocol are in Supporting Information (S9 Table).
...
\begin{table}[htbp]
\centering
\caption{Descriptive cross-setting synthesis of the expanded-hybrid increment, $\Delta$NB(M5$-$M1) at $p^*=0.30$ (primary $h=4$, 75th-percentile label). This table is a descriptive expanded-hybrid synthesis, not a primary inferential comparison: M5 is the expanded sensitivity hybrid (the planned primary hybrid is M4), 95\% CIs are percentile bootstrap and pointwise, and no cross-country interaction or heterogeneity test was performed. The estimates therefore should not be interpreted as establishing a between-setting difference. The settings differ in period, spatial units, sample size, and climate-feature implementation.}
...
The limited decision value of the climate-only models should not be read as climate being irrelevant to transmission. Recent case counts already summarize upstream processes, including climate effects, so over short-to-medium horizons a recent-case baseline is a demanding but appropriate comparator, and even a canonical distributed-lag nonlinear climate model did not exceed it at the primary threshold. The hybrid results are the key nuance. For the planned primary hybrid (M4), $\Delta\mathrm{NB}(\text{M4}-\text{M1})$ at $p^*=0.30$ was $-0.008$ (95\% CI $-0.028$ to $+0.013$) in Sri Lanka and $+0.0097$ (95\% CI $+0.0048$ to $+0.0150$) in the Colombia 2022-only secondary subset, while in the full Colombia test period the planned-primary M4 also exceeded M1 (net benefit 0.129 versus 0.117). For the expanded sensitivity hybrid (M5), the M5$-$M1 estimate was $+0.0081$ (95\% CI $-0.0012$ to $+0.0181$) in Sri Lanka and $+0.0188$ (95\% CI $+0.0117$ to $+0.0260$) in Colombia. The planned primary Sri Lanka contrast (M4$-$M1) did not exceed surveillance, whereas the expanded-sensitivity M5$-$M1 contrast was a small positive point estimate whose interval included zero. Two author-authorized secondary sensitivities, included to support rather than replace the primary findings, were consistent with this interpretation: the additional sensitivities did not materially alter the study's interpretation, in that the 2022-only Colombia estimate was modestly smaller than the full-period estimate, and the Sri Lanka few-cluster wild-cluster-bootstrap-t uncertainty intervals were similar to the original percentile intervals (Supporting Information). The original percentile-bootstrap intervals remain the primary reported uncertainty, the Colombia 2020--2022 analysis remains the primary replication, and neither sensitivity was treated as a primary analysis. No between-country heterogeneity test was performed, so the two settings are best read as complementary rather than contradictory and do not establish a difference: climate value may be conditional on setting, surveillance dynamics, and outcome and threshold definitions, and is most defensible as an incremental signal within a hybrid system that requires local evaluation. The magnitude of the Colombia increment was modest: at $p^*=0.30$ it corresponds to approximately 1.9 net true-positive equivalents (approximate interval 1.2 to 2.6) per 100 municipality-weeks, or, at a fixed number of true-positive detections, approximately 4.4 fewer false-alert equivalents (approximate interval 2.7 to 6.1) per 100 --- decision-analytic representations of the same net-benefit difference, not observed public-health outcomes.
...
\noindent All items below exist as standalone files transcribed from frozen committed reports and the finalized audited secondary-sensitivity outputs (no recomputation). Checklists S1--S3 and the inventory S10 remain drafts pending author finalization; S4--S9, S11, and S12 are complete and awaiting author approval. The two secondary sensitivities summarized in S9(c), S11, and S12 are pointwise, multiplicity-unadjusted robustness checks and are not primary analyses. No item is submission-final until the declarations and checklists are completed (see the Supporting-Information status record and \texttt{submission\_blockers\_v12.md}).\\[0.4em]
