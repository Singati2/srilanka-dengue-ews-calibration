...
\fancyhead[L]{\scriptsize\itshape Internal review copy}
\fancyhead[R]{\scriptsize Climate-informed dengue warning beyond surveillance}
\fancyfoot[R]{\footnotesize\thepage}
\renewcommand{\headrulewidth}{0pt}
\doublespacing
% Line numbers removed at author request (standing preference for review/reading copies).
% NOTE: PLOS NTD submission requires continuous line numbers; re-enable \linenumbers only for the submission build.
% \linenumbers
...
We conducted a retrospective decision-evaluation of dengue early-warning models in Sri Lanka (primary, RDHS-week, 2018--2025) and Colombia (external framework replication), comparing a recent-case surveillance baseline (M1) with climate-only, structured-climate, and hybrid climate--surveillance models. Models were evaluated at a four-week horizon by discrimination, calibration, recalibration, and decision-curve net benefit at a primary reference threshold of $p^*=0.30$. In Sri Lanka, the recent-surveillance benchmark (M1) exceeded the climate-only models (M2, M3) on net benefit, while the M5$-$M1 net-benefit estimate was $+0.0081$ (95\% CI $-0.0012$ to $+0.0181$). All M0--M3 models under-predicted in the higher-prevalence test period; recalibration reduced this error without changing the ranking. In Colombia, the corresponding estimate was $+0.0188$ (95\% CI $+0.0117$ to $+0.0260$), with discrimination gain $\Delta$AUC $+0.040$. At $p^*=0.30$ this corresponds to approximately 1.9 net true-positive equivalents per 100 municipality-weeks, or equivalently approximately 4.4 fewer false-alert equivalents per 100---decision-analytic representations, not observed public-health outcomes. In Colombia the increment was approximately flat across 1--12-week horizons rather than increasing with lead time, and Sri Lanka likewise showed no horizon-increasing decision value. No cross-country heterogeneity test was performed; therefore, the study does not establish a difference between settings.
...
Dengue early-warning systems often use weather and climate data to anticipate weeks of elevated dengue activity, and they are usually judged by how well they separate high-risk from low-risk weeks. Here, an alert week was one in which dengue activity exceeded a locally defined historical level, not an official outbreak declaration. Health agencies already track recent dengue case counts, which carry strong short-term information about rising risk. We asked a more practical question: does adding climate information improve the alert decisions agencies actually make, beyond what recent case counts already tell them? We evaluated climate-informed dengue models in Sri Lanka and Colombia against a recent-case surveillance benchmark, judging them by whether their predicted risks were well calibrated and improved alert decisions, not only by ranking accuracy. We found that recent surveillance was hard to beat, and climate alone added little. Combining climate with recent cases gave a small improvement whose magnitude and precision varied across the two evaluated datasets, and it did not grow with longer lead times. The study did not formally test whether the hybrid estimates differed between settings, so each result requires local interpretation. Climate is best used as a complement to surveillance, not a replacement.
...
\end{tikzpicture}
\caption{Study pipeline and WER-to-ISO week alignment. The primary linkage used reporting dates to map WER issues to ISO epidemiological weeks before joining dengue, climate, population, and spatial data; the key correction is that the WER issue number does not equal the ISO climate week. Box color and fill encode object category (data sources, preprocessing, integrated table, model families, evaluation, synthesis); category labels in the boxes provide a redundant, grayscale-legible encoding.}
\label{fig:pipeline}
\end{figure}
...
The Python DLNM-style and canonical R \texttt{dlnm} climate specifications produced nearly identical performance: their pairwise $\Delta$AUC and $\Delta$NB at $p^*=0.30$ were near zero, with 95\% CIs spanning zero. Both climate specifications remained below the recent-surveillance benchmark M1. Comparing the canonical R DLNM model with M1 yielded $\Delta$AUC $-0.038$ (95\% CI $-0.066$ to $-0.008$) and $\Delta$NB $-0.025$ at $p^*=0.30$ (95\% CI $-0.041$ to $-0.006$), both favoring M1 (M1 net benefit 0.137 versus 0.112 for the canonical R DLNM) (Table~\ref{tab:dlnmhybrid}). Among the hybrids, the primary hybrid M4 did not exceed M1: $\Delta$AUC(M4$-$M1) was $+0.013$ (95\% CI $-0.020$ to $+0.046$) and $\Delta$NB(M4$-$M1) at $p^*=0.30$ was $-0.008$ (95\% CI $-0.028$ to $+0.013$), favoring M1. The more fully specified hybrid M5 (adding seasonal harmonics and geographic fixed effects to M4) reached AUC 0.772, PR-AUC 0.667, Brier 0.180, and net benefit 0.145 at $p^*=0.30$ (M1: 0.752, 0.652, 0.191, 0.137); the M5$-$M1 net-benefit estimate was $+0.0081$ (95\% CI $-0.0012$ to $+0.0181$), so M5 had higher discrimination and a higher net-benefit point estimate than M1, with a confidence interval that extended below zero (Table~\ref{tab:dlnmhybrid}; see the cross-setting summary).

\begin{table}[htbp]
\centering
\caption{Sri Lanka climate-only, distributed-lag, and hybrid model comparison (test period, $h=4$, 75th-percentile label). Net benefit is shown at representative thresholds; M4 is the primary hybrid and M5 a more fully specified hybrid (adding seasonal harmonics and geographic fixed effects).}
...
M4 hybrid (primary) & 0.764 & 0.193 & 0.128 & 0.087 \\
...
\noindent In the primary common-complete analysis, M2 and M3 had net benefit close to alert-all ($\approx$0.107) and below M1. (The larger secondary climate-only row set is reported once in the Results above; the two analyses use different denominators and are not combined.)
...
\begin{axis}[
    ...
    symbolic x coords={M0,M1,M2,M3,M4,M5}, xtick=data, ymin=0.5, ymax=0.80, ytick={0.5,0.6,0.7,0.8},
...
\addplot[fill=okBlue!45, draw=okBlue!80!black, postaction={pattern=north east lines}] coordinates {(M0,0.513)(M1,0.685)(M2,0.556)(M3,0.564)(M4,0.699)(M5,0.726)};
\end{axis}
...
\end{axis}
\end{tikzpicture}
\caption{Colombia model-ladder discrimination and net benefit ($h=4$, common-complete test set, $n=13{,}361$). (A) AUC and (B) net benefit at $p^*=0.30$ for models M0--M5. Panel~A is anchored at the no-skill value (AUC${}=0.5$), which is the meaningful baseline for discrimination, and exact three-decimal AUC values are labeled above each bar. Panel B uses a zero-based axis so that the alert-none reference (net benefit $=0$) is the baseline and the alert-all reference ($\approx$0.107, dashed line) and the small between-model differences are shown in true proportion; exact three-decimal values are labeled above each bar. Climate-only models (M2, M3) had net benefit close to alert-all and below M1; M5 had the highest AUC and net benefit. Values are transcribed from Table~\ref{tab:colombia}; bars use redundant hatch patterns for grayscale legibility.}
\label{fig:colombia}
\end{figure}
...
% references_v6.bib; renders without BibTeX.
\begingroup
\setstretch{1.0}
...
\bibitem{OpenDengue2024} OpenDengue. Global dengue data, Temporal extract, version 1.3 [dataset]. figshare; 2025. doi:\,10.6084/m9.figshare.24259573.
...
\noindent The following Supporting Information items exist as draft internal materials and are not yet submission-ready; completed checklists are pending author confirmation (see the submission-blockers and Supporting-Information status records).\\[0.3em]
\noindent\textbf{S1 Checklist.} STROBE checklist for observational studies (draft).\\
\textbf{S2 Checklist.} TRIPOD+AI reporting checklist for prediction models (draft internal aid).\\
\textbf{S3 Table.} PROBAST risk-of-bias domain assessment (draft internal aid).\\
\textbf{S4 Text.} Analysis-plan and addendum chronology (prespecified versus documented extensions; draft).\\
...
\textbf{S6 Table.} Per-analysis bootstrap protocol (resampling unit, $B$, seed, interval method, degenerate-resample handling, failure count, pointwise/simultaneous and multiplicity status).\\[0.3em]
\noindent The following items are planned but \emph{not yet compiled as standalone Supporting Information files}; their values are transcribed from frozen committed reports and are not provided with this review copy: a Sri Lanka sensitivity grid (linkage, climate lag, late-2021 anomaly); a Sri Lanka targeted-value regime table; a Colombia exceedance-threshold (75th/80th/90th) and horizon-sensitivity table; and a reproducibility artifact inventory with checksums.
