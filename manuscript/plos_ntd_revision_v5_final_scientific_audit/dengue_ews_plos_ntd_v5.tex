...
\doublespacing
% Line numbers disabled at author request for this review copy.
% NOTE: PLOS NTD submission requires continuous line numbers; re-enable \linenumbers before submitting.
% \linenumbers
...
The Python DLNM-style and canonical R \texttt{dlnm} climate specifications produced nearly identical performance: their pairwise $\Delta$AUC and $\Delta$NB at $p^*=0.30$ were near zero, with 95\% CIs spanning zero. Both climate specifications remained below the recent-surveillance benchmark M1. Comparing the canonical R DLNM model with M1 yielded $\Delta$AUC $-0.038$ (95\% CI $-0.066$ to $-0.008$) and $\Delta$NB $-0.025$ at $p^*=0.30$ (95\% CI $-0.041$ to $-0.006$), both favoring M1 (M1 net benefit 0.137 versus 0.112 for the canonical R DLNM) (Table~\ref{tab:dlnmhybrid}). Among the hybrids, the primary hybrid M4 did not exceed M1: $\Delta$AUC(M4$-$M1) was $+0.013$ (95\% CI $-0.020$ to $+0.046$) and $\Delta$NB(M4$-$M1) at $p^*=0.30$ was $-0.008$ (95\% CI $-0.028$ to $+0.013$), favoring M1. The steelman hybrid M5 reached AUC 0.772, PR-AUC 0.667, Brier 0.180, and net benefit 0.145 at $p^*=0.30$ (M1: 0.752, 0.652, 0.191, 0.137); the M5$-$M1 net-benefit estimate was $+0.0081$ (95\% CI $-0.0012$ to $+0.0181$), so M5 had higher discrimination and a higher net-benefit point estimate than M1, with a confidence interval that extended below zero (Table~\ref{tab:dlnmhybrid}; see the cross-setting summary).

\begin{table}[htbp]
\centering
\caption{Sri Lanka climate-only, distributed-lag, and hybrid model comparison (test period, $h=4$, 75th-percentile label). Net benefit is shown at representative thresholds; M4 is the primary hybrid and M5 the steelman hybrid reference.}
...
\begin{axis}[
    ...
    symbolic x coords={M0,M1,M2,M3,M4,M5}, xtick=data, ymin=0.45, ymax=0.80,
...
\end{axis}
\end{tikzpicture}
\caption{Colombia model-ladder discrimination and net benefit ($h=4$, common-complete test set, $n=13{,}361$). (A) AUC and (B) net benefit at $p^*=0.30$ for models M0--M5. Panel B uses a zero-based axis so that the alert-none reference (net benefit $=0$) is the baseline and the alert-all reference ($\approx$0.107, dashed line) and the small between-model differences are shown in true proportion; exact three-decimal values are labeled above each bar. Climate-only models (M2, M3) had net benefit close to alert-all and below M1; M5 had the highest AUC and net benefit. Values are transcribed from Table~\ref{tab:colombia}; bars use redundant hatch patterns for grayscale legibility.}
\label{fig:colombia}
\end{figure}
...
% references_v5.bib; renders without BibTeX.
\begingroup
\setstretch{1.0}
