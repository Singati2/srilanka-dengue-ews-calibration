...
DEVINCL=json.load(open("/home/mpcrlab/srilanka-dengue-ews-calibration/analysis/v18_bootstrap_b1000/SL_devinclusive_B1000.json"))["development_inclusive"]
...
# Panel B: matched increment --- development-inclusive interval is the visual hero
di_raw=DEVINCL["raw_matched"]; di_rec=DEVINCL["recal_matched"]
# conditional bands: light, explicitly secondary/optimistic
axB.fill_between(grid,lo_r,hi_r,color='#1f77b4',alpha=0.10)
axB.fill_between(grid,lo_c,hi_c,color='#d62728',alpha=0.10)
axB.plot(grid,inc_raw,'-',color='#1f77b4',lw=1.3,alpha=0.75,label='Increment, raw (point)')
axB.plot(grid,inc_recal,'-',color='#d62728',lw=1.3,alpha=0.75,label='Increment, past-only recal. (point)')
axB.axhline(0,color='k',lw=1.4,zorder=1)  # zero prominent
axB.axvline(0.30,color='0.75',lw=0.8,ls=':')
# development-inclusive 95% CIs at the reference threshold (the honest interval; both include 0)
for xoff,ci,pt,col,lab in [(-0.006,di_raw["ci"],di_raw["point"],'#1f77b4','raw'),
                            (0.006,di_rec["ci"],di_rec["point"],'#d62728','recal.')]:
    x=0.30+xoff
    axB.plot([x,x],ci,color=col,lw=3.2,solid_capstyle='round',zorder=5)
    axB.plot([x-0.006,x+0.006],[ci[0],ci[0]],color=col,lw=1.6,zorder=5)
    axB.plot([x-0.006,x+0.006],[ci[1],ci[1]],color=col,lw=1.6,zorder=5)
    axB.plot(x,pt,'o',color=col,ms=5,mec='k',mew=0.6,zorder=6)
axB.annotate('Development-inclusive 95% CI\n(refits both models; includes 0)',
    xy=(0.306,di_rec["ci"][1]),xytext=(0.325,0.035),fontsize=6.6,color='0.15',ha='left',va='top',
...
axB.legend(fontsize=6.6,frameon=False,loc='upper left'); axB.set_xlim(0.05,0.50)
plt.tight_layout()
plt.savefig(FIGDIR+"/Fig4.pdf",bbox_inches='tight'); plt.savefig(FIGDIR+"/../Fig4_v23_preview.pdf",bbox_inches='tight')
