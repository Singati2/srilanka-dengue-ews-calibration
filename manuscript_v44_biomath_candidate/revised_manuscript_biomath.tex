...
% Marker for analyses deferred pending the registered refit.
...
\paragraph{Measuring what the folds cost.} Re-evaluating the published ladder under these folds requires refitting each model, hence each design matrix, and has not been run\,\REGBLOCK; a spatially cross-validated model set against a temporally split comparator would be an artifact of the schemes rather than a comparison. We therefore measured the fold effect as a \emph{within-model} contrast on the geomatics-only model, where features and labels are held locally: the same model, same features, same 3{,}926 held-out rows, with only the validation scheme changing. This is a stricter instrument than the cross-model version, because nothing else varies.
