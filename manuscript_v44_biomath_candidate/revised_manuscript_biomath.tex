...
% =====================================================================
% BIOMATH REFRAME CANDIDATE (writing/formalization only; NOT for submission).
% Derived by copying the canonical manuscript_v44/revised_manuscript.tex
% (branch agent/v44-round4-geomatics-execution @ 4b3287c; SHA256 08fad12b...)
% and INSERTING a decision-theoretic formalization layer:
%   - a "Mathematical and decision-analytic framework" Methods subsection
%     (outcome, nested information sets, incremental estimand, probabilistic
%      prediction + calibration operator, threshold rule, net benefit,
%      proper scoring rules, conditional vs development-inclusive uncertainty,
%      forecast-origin information admissibility, spatial exposure operator);
%   - a notation table; a nested-information Introduction framing (the four
%     levels: association / standalone / incremental / decision value);
%     question-form Results headings; figure-safe \safeincludegraphics.
% NO number, model, metric, interval, bootstrap, calibration curve, threshold,
% horizon, split, or hierarchy was changed. M6/WP4/WP5 geomatics results are
% NOT imported (firewall). The spatial-exposure operator and any geomatics
% incremental estimand appear ONLY as framework / PENDING, never as results.
% Every number traces to the canonical v44 source; see BIOMATH_NUMERIC_CROSSWALK.md.
% ---------------------------------------------------------------------
% Original v44 provenance note (retained): Version 25 (submission closeout),
% PLOS Global Public Health candidate; corrected cross-setting interpretation;
% every number traces to the frozen v23 tables/results; secondary diagnostics,
% ladders, and provenance are in the Supporting Information at the end.
% Build flags: \def\linenumberedcopy{} (line numbers); \def\grayscalecopy{} (gray).
% =====================================================================
...
% Figure-safe include: submission_figs/*.pdf are gitignored and absent in this
% checkout. Render a labeled placeholder instead of failing, so the candidate
% compiles for internal review without creating or altering any figure.
\newcommand{\safeincludegraphics}[2][]{%
  \IfFileExists{#2}{\includegraphics[#1]{#2}}{%
    \fbox{\parbox{0.82\linewidth}{\centering\footnotesize [figure not committed to repo --- supply from submission\_figs/ to render; no figure was created or modified]}}}}
...
\usepackage{xcolor}
\ifdefined\grayscalecopy \selectcolormodel{gray} \fi
\IfFileExists{setspace.sty}{\usepackage{setspace}}{\providecommand{\setstretch}[1]{}\providecommand{\doublespacing}{}\providecommand{\singlespacing}{}}
\usepackage[numbers,sort&compress]{natbib}
\usepackage[colorlinks=true,linkcolor=black,citecolor=black,urlcolor=black]{hyperref}
\IfFileExists{lineno.sty}{\usepackage{lineno}}{\providecommand{\linenumbers}{}\providecommand{\nolinenumbers}{}}
...
% Marker for every claim that rests on an independent rebuild of the linked
% analysis table rather than on the frozen archive. Such claims are sensitivity
% analyses and must never be presented as the registered re-runs, which remain
% outstanding. Resolve to an explicit footnote before submission.
...
% TITLE PAGE
% =====================================================================
\IfFileExists{fancyhdr.sty}{\thispagestyle{fancy}}{}
\begin{center}
{\Large \textbf{The incremental decision value of climate information beyond recent surveillance for short-lead dengue elevated-activity forecasting: an exploratory matched-comparator evaluation}}\\[1.0em]
...
\vspace{1em}
\begin{center}
\fbox{\parbox{0.9\linewidth}{\centering\footnotesize \textbf{Internal biomath-reframe candidate --- not for submission.} This is a writing- and mathematical-formalization pass over the frozen canonical manuscript (\texttt{manuscript\_v44/revised\_manuscript.tex}). No model, metric, confidence interval, bootstrap, calibration curve, threshold, horizon, split, or analysis-hierarchy was changed; every number is reused verbatim (see \texttt{BIOMATH\_NUMERIC\_CROSSWALK.md}). Geomatics M6/WP4/WP5 results are not imported; the spatial-exposure operator and any geomatics incremental estimand appear only as framework or pending. Author-owned submission items (ethics, ORCIDs, funding, competing interests, archival DOI/license) remain pending as flagged in the Declarations.}}
\end{center}
...
\subsection{Mathematical and decision-analytic framework}\label{sec:framework}
This subsection states the decision-theoretic objects used throughout. It introduces no new data, model, or estimate; it formalizes the evaluation actually carried out, and every symbol maps to an existing empirical object (Table~\ref{tab:notation}). It is deliberately not a theorem apparatus: the mathematics clarifies estimands and validity conditions.

\paragraph{Forecast outcome and horizon.}
Let $i$ index spatial units and $t$ index epidemiological weeks, and fix a forecast horizon $h$ (primary $h=4$ weeks). Writing $\tau_i$ for unit $i$'s training-period 75th percentile of weekly incidence (a 90th-percentile variant is a stricter-outcome sensitivity), the elevated-activity outcome is
\[
Y_{i,t+h}=\mathbf 1\{\text{incidence}_{i,t+h}>\tau_i\},
\]
a training-defined threshold exceedance $h$ weeks ahead, joined by calendar date rather than positional shift. $Y$ is a statistical elevated-activity classification (test-period prevalence $\approx0.34$--$0.38$), not an official outbreak declaration.

\paragraph{Information sets.}
For unit $i$ and week $t$, let $\mathcal I^{S}_{it}$ be recent-surveillance information available by week $t$, $\mathcal I^{C}_{it}$ climate information available by week $t$, and $\mathcal I^{SC}_{it}=\mathcal I^{S}_{it}\cup\mathcal I^{C}_{it}$ their union. The ladder maps to sets without altering any model definition: M0 uses seasonal/calendar information; M1 is $\mathcal I^{S}$ (Colombia: recent-incidence lags only; Sri Lanka: additionally seasonal harmonics and RDHS fixed effects); M2 is $\mathcal I^{C}$; M3 a structured $\mathcal I^{C}$; M4 the planned primary hybrid; and M5 an expanded hybrid adding seasonal and geographic structure. The matched no-climate comparator $\text{M5}_{\text{no-climate}}$ retains M5's case-history, seasonal, and geographic blocks and removes \emph{only} the climate block, so $\mathcal I^{S}\subseteq\mathcal I^{SC}$ holds for the pair $(\text{M5}_{\text{no-climate}},\text{M5})$; the shared row universe, estimator, tuning rule, and evaluation machinery are documented in \texttt{MATCHED\_COMPARATOR\_STRUCTURE.md}.

\paragraph{Probabilistic prediction.}
For model $m$ using information set $\mathcal I^{(m)}_{it}$, the predicted alert probability is
\[
p^{(m)}_{it}=P\!\left(Y_{i,t+h}=1\mid \mathcal I^{(m)}_{it}\right),
\]
a predictive probability from the fitted statistical model, not a mechanistic transmission probability.

\paragraph{Incremental value of climate (central estimand).}
Let $\{V_k\}_{k\in\mathcal K}$ be a family of evaluation functionals, each \emph{oriented so that larger is better}: $V_{\mathrm{AUC}}=\mathrm{AUC}$, $V_{\mathrm{NB},p^*}=\mathrm{NB}(p^*)$, $V_{\mathrm{Brier}}=-\mathrm{Brier}$, and $V_{\mathrm{NLL}}=-\mathrm{NLL}$ (orientation stated once here and used throughout). The central estimand is the metric-specific matched climate increment
\[
\Delta V_{C,k}=V_k(\mathcal I^{SC})-V_k(\mathcal I^{S}),
\]
estimated by the matched climate ablation (M5 versus $\text{M5}_{\text{no-climate}}$; see the matched-ablation subsection and \texttt{MATCHED\_COMPARATOR\_STRUCTURE.md}). The question is not ``can climate predict dengue?'' but ``what additional decision-relevant value remains attributable to climate once recent surveillance and the matched non-climate structure are already available?'' Read across $k$, $\Delta V_{C,k}$ separates ranking, probability accuracy, and decision value (Results). The design-locked contrast M4$-$M1 is \emph{non-nested} (M4 and M1 differ in more than climate) and therefore does not isolate the climate block; only the matched pair does.

\paragraph{Calibration operator.}
A recalibration map acts on the logit of a raw prediction,
\[
\widetilde p_{it}=\operatorname{logit}^{-1}\!\left[\alpha_t+\beta_t\,\operatorname{logit}(p_{it})\right];
\]
the primary rolling intercept-only procedure fixes $\beta_t=1$ and estimates $\alpha_t$ from outcomes observable before the prediction week. A strictly monotone recalibration preserves the risk ranking and therefore leaves AUC unchanged, but can change calibration, the threshold classifications, and net benefit at a fixed $p^*$; intercept-only recalibration corrects average over/under-prediction but not calibration slope or nonlinear miscalibration. Idealized calibration is $P(Y=1\mid \hat p=p)=p$; the reported CITL and slope measure departures from it (intercept: systematic over/under-prediction; slope: dispersion of predictions).

\paragraph{Threshold decision rule.}
Given a threshold probability $p^*$, the alert action is
\[
a^{(m)}_{it}=\mathbf 1\{p^{(m)}_{it}\ge p^*\}\equiv\delta_{p^*}(p^{(m)}_{it}),\qquad \delta_{p^*}(p)=\mathbf 1(p\ge p^*),
\]
so each model induces a map $\mathcal I_{it}\to p_{it}\to a_{it}$ from information to probability to action. $p^*$ encodes a decision trade-off; it was neither stakeholder-elicited nor cost-derived here (illustrative $p^*=0.30$).

\paragraph{Net benefit and default strategies.}
With $\mathrm{TP}$ and $\mathrm{FP}$ the true- and false-positive alert counts and $N$ observations,
\[
\mathrm{NB}(p^*)=\frac{\mathrm{TP}}{N}-\frac{\mathrm{FP}}{N}\,\frac{p^*}{1-p^*},
\]
where $p^*/(1-p^*)$ is the false-alert weight in true-positive-equivalent units implied by the threshold. For test-set prevalence $\pi$ the alert-all strategy has $\mathrm{NB}_{\mathrm{all}}(p^*)=\pi-(1-\pi)\,p^*/(1-p^*)$, so $\mathrm{NB}_{\mathrm{all}}(p^*)=0\iff p^*=\pi$; this is a mathematical property of the threshold and prevalence (it explains, rather than newly estimates, why alert-all was competitive at low thresholds and near zero close to $p^*\approx0.34$ when Sri Lanka test prevalence was $\approx0.336$). A model has decision value at $p^*$ only if its net benefit exceeds both alert-all and alert-none. The incremental decision value of climate is $\Delta\mathrm{NB}_C(p^*)=\mathrm{NB}_{SC}(p^*)-\mathrm{NB}_{S}(p^*)$, the $V=\mathrm{NB}$ instance of $\Delta V_C$. We make no monetary or health-utility conversion (no cost elicitation was performed).

\paragraph{Metric roles.}
The four functionals answer different questions and are not interchangeable: AUC (ranking), the strictly proper Brier and negative-log-loss scores (average probability accuracy; full definitions in S18 Text), calibration (agreement of predicted risks with observed frequencies; treated separately below, as it is not a single larger-is-better scalar), and net benefit (the threshold-specific alert trade-off). A gain on one need not transfer to another; in particular a small $\Delta\mathrm{AUC}$ or proper-score gain need not yield $\Delta\mathrm{NB}>0$. Each climate increment is reported as the corresponding $\Delta V_{C,k}$.

\paragraph{Conditional versus development-inclusive uncertainty.}
Two resampling targets are distinguished. \emph{Conditional} intervals hold the developed predictions fixed ($\hat p_i$ fixed) and resample the evaluation structure, reflecting test-set sampling only. \emph{Development-inclusive} intervals refit both models within each cluster resample, following $D^{(b)}\to\hat f^{(b)}\to\hat p^{(b)}\to\Delta^{(b)}$, and therefore address a broader uncertainty target that includes model redevelopment (subject to the exact resampling scheme; department fixed-effect columns are held fixed in Colombia, so the procedure is development-inclusive for the case-history, seasonal, and climate blocks but not for those fixed effects). Development-inclusive intervals are the primary basis for judging robustness.

\paragraph{Forecast-origin admissibility.}
An information item $X_j$ is admissible at origin $t$ only if it would have been available by then: $X_j\in\mathcal I_t \iff \tau_{\mathrm{avail}}(X_j)\le t$. Concretely, climate predictors use only origin-week (lag~0) and earlier values (for $h=4$ the climate-to-outcome window spans 4--12 weeks), and, because the stored WER label is the issue number $r$ rather than the ISO week, surveillance is joined through the reporting-date map $w(d(r))$ rather than assuming $r=w$ (a one- or two-week indexing error would shift a feature intended at lag $\ell$ to lag $\ell+k$). This admissibility principle also governs remotely sensed inputs, whose release latency is not modeled here; that is a matter for the separate, pending geomatics workstream, and no such number is imported.

\begin{table}[htbp]
\centering
\caption{Notation for the decision-analytic framework. Every symbol maps to an existing empirical object; no symbol denotes a newly estimated quantity.}
\label{tab:notation}
\footnotesize\setstretch{1.0}
\begin{tabularx}{\textwidth}{@{}llY@{}}
\toprule
Symbol & Meaning & Empirical object \\
\midrule
$i$ & spatial unit & RDHS division (SL) / municipality (CO) \\
$t$ & forecast-origin week & epidemiological week \\
$h$ & forecast horizon & primary $h=4$ weeks \\
$Y_{i,t+h}$ & future elevated-activity indicator & training-defined 75th-pct exceedance \\
$\mathcal I^{S},\mathcal I^{C},\mathcal I^{SC}$ & surveillance / climate / hybrid information & recent-case, climate feature blocks; their union \\
$p^{(m)}_{it}$ & predicted alert probability & fitted-model output \\
$\widetilde p^{(m)}_{it}$ & recalibrated probability & rolling intercept-only (SL) / Platt (CO) \\
$p^*$ & decision threshold & illustrative $p^*=0.30$ \\
$a^{(m)}_{it},\ \delta_{p^*}$ & alert action / decision rule & thresholded prediction \\
$\pi$ & test-set alert prevalence & 0.336 (SL test), 0.375 (CO test) \\
$\mathrm{NB}$ & net benefit & decision-curve endpoint \\
$V_k,\ \Delta V_{C,k}$ & oriented value functional / metric-specific climate increment & $k\in\{\mathrm{AUC},\mathrm{NB},-\mathrm{Brier},-\mathrm{NLL}\}$; $V_k(\mathcal I^{SC})-V_k(\mathcal I^{S})$ \\
$\tau_{\mathrm{avail}}$ & availability time (admissibility: $\le t$) & WER$\to$ISO map $w(d(r))$; admissible lags \\
$\mathrm{TP},\mathrm{FP},N$ & true/false positives, sample size & alert counts; unit-weeks evaluated \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{Exposure construction as an analyst degree of freedom}\label{sec:methods-wp5}
Areal climate exposure is a construction, not an observation: a gridded field must be collapsed onto administrative polygons, and the collapse requires choosing which cells to admit and what to weight them by. In the notation of Section~\ref{sec:framework}, this choice does not change $\mathcal I^{C}$ as a concept but does change how it is realized numerically, so it is a candidate explanation for miscalibration that would otherwise be attributed to the models --- the modifiable areal unit problem in its weighting form~\cite{Openshaw1984,Fotheringham1991}. The primary analysis used unweighted administrative-unit means, as described above; this subsection describes a parallel sensitivity ladder built to test whether that choice matters.

\paragraph{What the ladder is, and what it is not.} The ladder is a \emph{separate, parallel} exposure build, not a rebuild of the primary exposure table. Precipitation is identical to the primary build (CHIRPS v2 daily at its native $0.05^{\circ}$). Temperature and humidity, however, were built on ERA5 single-level reanalysis at $0.25^{\circ}$~\cite{Hersbach2020} accessed through an analysis-ready cloud-optimized mirror~\cite{Carver2023ARCO}, because the $0.1^{\circ}$ ERA5-Land product used for the primary analysis requires a Copernicus Climate Data Store account that the geospatial analysis environment did not have. \textbf{Temperature results from this ladder therefore describe a coarser analogue of the primary exposure, not the primary exposure itself.} Relative humidity was computed per cell per hour by the Alduchov--Eskridge Magnus formulation~\cite{AlduchovEskridge1996} before any averaging, as in the primary build.

The substitution attenuates the very effect being measured, and the attenuation was quantified from the elevation model and population raster alone, before any exposure table was built: at $0.25^{\circ}$ the population-weighted construction recovers 65.3\% of the elevation displacement it recovers at $0.1^{\circ}$ (91.7\%), and the effective number of contributing cells per district falls from 21.3 (area) and 11.8 (population) to 5.3 and 3.6. The bias runs \emph{toward the null}, so every temperature displacement below is a conservative floor. The same attenuation makes district-level attribution unreliable on the A$'\to$B temperature rung in the most attenuated districts (Ratnapura retains 6.7\% of its predicted shift), and we make no claim about \emph{which} districts warm under population weighting on that rung.

\paragraph{The three-build ladder.} Each rung adds exactly one refinement, holding the weather fields, geometry, outcome labels, temporal split and reference threshold fixed. \textbf{Build A} is the incumbent area-weighted construction: every cell touching a polygon is admitted (an \texttt{all\_touched} mask), averaged with equal weight. \textbf{Build A$'$} replaces that mask with fractional-area weighting, separating \emph{which cells are admitted} from \emph{what they are weighted by}. \textbf{Build B} re-weights the same cells by the population they contain. \textbf{Build C} additionally corrects temperature for the elevation difference between the reanalysis orography and the population's true elevation. All four tables carry the same 10{,}842 keys (26 RDHS $\times$ 417 ISO weeks) and are row-aligned, so they differ by construction alone.

All weighting used an equal-area projection (EPSG:32644); no weight was computed in geographic degrees. Population weights came from WorldPop 100\,m UN-adjusted constrained rasters~\cite{Tatem2017,WorldPop2025}, assigned to (district, cell) pairs by pixel centre and normalised within district, so a district's national total cancels and only the \emph{shape} of its population surface can affect exposure. WorldPop UN-adjusted terminates at 2020 against a window ending in 2025; rather than impute later vintages we held the 2020 field constant and measured the drift it induces ($\approx$1.1\% of a typical weight over 2018--2020), reporting it as a bounded extrapolation.

\paragraph{Build C, and why it needs two elevations.} A lapse correction requires both the true population-weighted elevation and the elevation the reanalysis implicitly assumes. The latter came from ERA5's own surface geopotential, verified invariant across hours and years. Correcting to a digital elevation model alone --- the natural shortcut --- assumes the reanalysis already sits at true mean elevation, which is exactly what fails in the highlands: ERA5's orography over Sri Lanka peaks at 1{,}219\,m against the island's true 2{,}524\,m. Temperature and dewpoint were corrected at the environmental lapse rate ($6.5\,^{\circ}$C\,km$^{-1}$; $1.5\,^{\circ}$C\,km$^{-1}$ for dewpoint) with a saturation guard, then aggregated under Build B weights, so B$\to$C isolates terrain on top of population weighting. Because the correction is affine in elevation, correcting each fine pixel then aggregating is exactly equivalent to correcting the weighted-mean elevation, which we verified; humidity is the exception and was recomputed per cell per hour from corrected fields. Terrain came from Copernicus GLO-30~\cite{CopernicusDEM2021}. Precipitation carries no Build C rung: CHIRPS is gauge-calibrated observation, not a reanalysis with an internal orography to correct, and building an orographic-enhancement model would violate the study's no-new-model rule.

Build C was validated against station observations rather than accepted on construction. GHCN-Daily~\cite{Menne2012} provides six Sri Lankan stations reporting through 2025, one at Nuwara Eliya (1{,}880\,m) and the rest between 1.8 and 116\,m; station daily means were compared with ERA5 daily means on local (UTC+5:30) days. Because Build B rests on one modelled population surface, the weight field was also rebuilt at the same 2020 epoch under WorldPop R2025A and GHS-POP R2023A~\cite{Schiavina2023}, the latter produced by a different institution using a built-up-surface dasymetric method rather than a random forest. A fourth arm named in the analysis plan --- census district totals --- is degenerate and was struck rather than run: totals carry no within-district spatial information, so distributing them uniformly reproduces Build A exactly.

\paragraph{Decision sensitivity without refitting.} The registered form of this analysis refits the ladder under each build and reports $\Delta$AUC, $\Delta$calibration and $\Delta V_{C,k}$. We report it in two independent forms, the weaker one first, because that one needs nothing beyond the frozen predictions and so carries no substitution caveat at all. The question \emph{does the alert decision move?} does not require refitting, and is answered within a bounded envelope of three pieces. First, an exact model-free \emph{flip curve}: a held-out row can change alert status only if its predicted risk lies within the applied perturbation of $p^*$, so $\#\{|p-p^*|\le\delta\}$ bounds the flips attributable to \emph{any} perturbation of size $\delta$. Second, an observed \emph{ceiling}: the alert changes produced by moving from $\mathcal I^{SC}$ to $\mathcal I^{S}$ --- that is, by the estimand's own contrast --- computed on identical rows from the frozen M5 and $\text{M5}_{\text{no-climate}}$ predictions. Third, a \emph{transfer coefficient} $\beta$ relating exposure to predicted risk, from regressing $\mathrm{logit}(p_{\text{M5}})-\mathrm{logit}(p_{\text{M5-no-climate}})$ on the exposure variables, which needs predictions only. We estimated $\beta$ under four specifications (exposure alone; plus district fixed effects; plus seasonal harmonics; plus both) and report the range across all four, because the specification is not identified without the design matrices. Every estimated flip count is asserted against the exact flip curve. Uncertainty follows the study's established protocol: percentile intervals from 2{,}000 cluster-bootstrap resamples of the 26 RDHS units, seed 20260612~\cite{CameronGelbachMiller2008}.

\paragraph{Decision sensitivity by refitting, and the fourth arm it requires.}\REBUILD{} We additionally refitted the model under each build, which is the registered form of the analysis and answers what the envelope cannot: whether construction moves what the model \emph{scores}, not only what it \emph{decides}. Refitting requires the linked analysis table, which is not present in the analysis environment used here; the design-matrix, cross-basis and fitting code is, and reproduces the frozen fits. We therefore refitted on an independent rebuild of that table, built from the same weather windows and district weights and gated against the frozen run: refitted on the rebuild, M5 agrees with the frozen model's alert labels on 98.8\% of the 3{,}926 held-out rows and its predictions correlate 0.975 with the frozen ones (0.976 for the climate-free twin).\label{par:rebuild} Results from it are a sensitivity analysis, not the registered re-run, which remains outstanding. Three models were refitted under each build: M5; a reduced specification without district fixed effects or harmonics; and $\text{M5}_{\text{no-climate}}$. The ladder was run with \emph{four} arms rather than three, because Build C makes two changes at once and only one is a construction effect: C-orography applies the lapse correction under area weights, isolating the reanalysis orography deficit, and C-full applies it under population weights, adding sub-grid population placement. Without the third arm the whole of Build C would be attributed to the modifiable areal unit, which the exposure decomposition above already shows it is not. Cross-basis knots were re-derived from each arm's training rows, with a fixed-knot variant as a sensitivity. Four gates were passed before any contrast was read, two of which test the machinery rather than the result: the Build A$'$ arm reproduces the rebuild's own committed predictions to \emph{exactly} zero, and $\text{M5}_{\text{no-climate}}$ is bit-identical across all four arms --- a model that cannot see the climate fields must not move when they are rebuilt. A \emph{swap-path null} rebuilds Build A$'$ through the identical machinery and refits, giving the flip count the pipeline produces when nothing changes (3 of 3{,}926 rows at $p^*=0.30$); labels and the temporal split were asserted invariant across arms.

\paragraph{Where effects fall in space.} To locate rather than merely quantify these effects we computed four per-unit quantities --- calibration slope and intercept (a within-unit logistic fit of the outcome on the logit of the held-out prediction), the unit's change in net benefit under $\mathcal I^{SC}\to\mathcal I^{S}$, and its exposure-driven flip rate --- and screened each against 15 geospatial modifiers fixed in advance from the study's risk-factor review (elevation, built-up fraction, NDVI, distance to permanent water, slope, topographic wetness, height-above-nearest-drainage, tree cover, cropland, land-cover diversity, land-cover edge density, surface-water fraction, population density, nighttime lights). The unit's own alert prevalence was carried through as a non-geospatial decoy, because outbreak burden is the most obvious alternative explanation and would otherwise be inherited silently by any correlated modifier. Associations are Spearman rank correlations with Benjamini--Hochberg control across the entire $4\times15$ grid at once~\cite{BenjaminiHochberg1995}; all 60 tests are reported, not the surviving subset. The analysis is exploratory-explanatory: with 26 units it maps where conclusions are least reliable, and no modifier may re-enter the ladder as an accuracy predictor.

\subsection{Spatially-honest cross-validation}\label{sec:methods-wp4}
The held-out evaluation in Section~\ref{sec:methods-wp5} and throughout is a single temporal split, which leaves the external-validation claim exposed to a spatial objection: neighbouring units share weather and outbreak timing, so a model trained on a district's neighbours has seen much of that district's signal. We addressed this with buffered leave-one-out cross-validation sized to Sri Lanka's 26 units~\cite{Roberts2017,Valavi2019}.

\paragraph{Fold construction and the leakage audit.} Each RDHS unit was held out in turn and the training set additionally excluded every unit within a buffer radius. Distance was measured \emph{boundary-to-boundary} in EPSG:32644, not centroid-to-centroid: adjacent Sri Lankan districts sit 28--119\,km apart by centroid while sharing a border, so a centroid buffer would apply a wildly uneven exclusion. Folds were built as a function of radius (0, 10, 25, 50, 75, 100, 150\,km, plus graph-hop variants) so the operative radius could be selected without rebuilding. Hop-based and kilometre-based buffers are not interchangeable --- one graph hop removes between 2 and 7 districts depending on location, applying an uneven buffer while appearing uniform --- and kilometre buffers are treated as primary. The spatial half of the leakage audit was asserted rather than assumed: for every radius and fold, zero train--test unit pairs fall within the buffer. Temporal honesty is enforced upstream, where a composite predictor is admissible for week $W$ only if its averaging window \emph{ended} before $W$ began; the constraints are orthogonal, so the scheme is spatio-temporal. The buffer radius was to be set from the residual spatial autocorrelation range: Moran's $I$ was computed per test week across distance bands on the adjacency graph against a permutation null (999 shuffles, seed 20260612), and an empirical variogram fitted over 0--400\,km, for the frozen M5 and $\text{M5}_{\text{no-climate}}$ and for the geomatics-only model.

\paragraph{Measuring what the folds cost.} A spatially cross-validated model set against a temporally split comparator would be an artifact of the schemes rather than a comparison, so the fold effect must be measured as a \emph{within-model} contrast: the same model, same features, same 3{,}926 held-out rows, with only the validation scheme changing. We ran that contrast on two models. The first is the geomatics-only model, whose features and labels are held locally and which carries no substitution caveat. The second is M5 itself\,\REBUILD, refitted under every fold on the independent rebuild described above (p.~\pageref{par:rebuild}) and reported as a sensitivity analysis rather than as the registered cross-model re-run. Running both was not redundancy: the price of a fold is a property of the model as well as of the geography, and the geomatics-only version alone gives the wrong sign at the operative radius. One caution governs the M5 arm. A held-out district's fixed effect is not estimable --- its dummy is all-zero in every training set, so the district is scored at the reference intercept whatever the buffer --- and every temporal-versus-fold contrast for that model therefore conflates losing the neighbours with losing the fixed effect. The matched random-district control carries the identical handicap, so only the buffered-minus-control gap separates them; it is the estimand throughout.

The design point is the control. Buffering removes spatial leakage \emph{and} training data, and both depress performance, so a raw buffered-versus-temporal comparison measures sample size and calls it leakage. Every buffered fold was matched against a \emph{random-district control} trained on an identical number of districts drawn without regard to geography (10 draws per radius); the buffered-minus-matched gap is the part attributable to geography, and the remainder is the price of a smaller training set. Across four arms this required 1{,}456 model fits. Two gates were passed before any contrast was read: the rebuilt outcome labels reproduce the frozen outcomes on all 3{,}926 test rows, and the baseline arm reproduces the committed model's predictions to $8.6\times10^{-15}$. Uncertainty used the same cluster bootstrap (2{,}000 replicates, districts resampled, seed 20260612), with the control ensemble averaged inside each replicate.

\paragraph{Power, and what it bounds.} With 26 units this is a low-powered design by construction, and no better-powered spatial companion is reported: a Colombia block cross-validation over approximately 1{,}000 municipalities was scoped and is not part of this study's evidence. The Sri Lanka spatial result therefore stands alone and is reported as a characterised limitation rather than a certified claim, with the power cost quantified in Table~\ref{tab:wp4-power}.
...
\subsection{Q1. How strong is the recent-surveillance benchmark? (Cohort and data alignment)}
...
\begin{figure}[htbp]
\centering
\safeincludegraphics[width=\textwidth]{submission_figs/Fig1.pdf}
...
\subsection{Q2. What incremental decision value remains after adding climate? Sri Lanka}
...
\begin{figure}[htbp]\centering\safeincludegraphics[width=0.98\textwidth]{submission_figs/Fig4.pdf}
...
\subsection{Q6. Does the increment persist across robustness, selection, and horizon?}\label{sec:robustness}
...
% DISCUSSION
% =====================================================================
\subsection{Q7. Is the increment robust to how climate information is operationalized, and to spatially-honest validation?}\label{sec:wp45}
Sections~\ref{sec:methods-wp5} and~\ref{sec:methods-wp4} pose two questions that the preceding analyses hold fixed by construction: whether $\Delta V_{C,k}$ depends on how $\mathcal I^{C}$ is realized numerically, and whether the held-out evaluation survives a validation scheme that respects spatial structure. Both are Sri Lanka only.

\paragraph{Exposure construction moves exposure.} Construction moved every variable measured, so the pre-specified honest-null clause fired on none of them (Table~\ref{tab:wp5-ladder}, Fig~\ref{fig:wp5f2}). Across 10{,}842 district-weeks, replacing area with population weighting displaced weekly rainfall by a mean absolute 3.40\,mm (7.9\% of the 42.9\,mm mean, largest single district-week 72.6\,mm), weekly mean temperature by 0.205\,$^{\circ}$C, weekly maximum temperature by 0.387\,$^{\circ}$C and relative humidity by 0.77 percentage points; the lapse correction moved weekly mean temperature by a further 0.306\,$^{\circ}$C. Four features matter more than the magnitudes. \emph{The mask is a footnote; the weights are the result}: exchanging the \texttt{all\_touched} mask for fractional-area weighting moved 1.8\% of mean rainfall against 7.9\% for the area-to-population step, so what a table is weighted by matters roughly four times more than which cells it admits --- and it is the weighting that studies leave unreported. \emph{The rungs act on different parts of the distribution}: population weighting moved the weekly \emph{maximum} temperature about twice as far as the mean (0.387 against 0.205\,$^{\circ}$C, maximum 2.76\,$^{\circ}$C), which is the relevant quantity for a process governed by thermal limits rather than averages. \emph{In the weeks that would trigger an alert the rainfall contrast grows in millimetres and shrinks in percent} (8.6\,mm / 5.4\% in the wettest decile; 12.9\,mm / 4.9\% in the wettest percentile), so the overall correlation between arms (0.9935 for rainfall) is not the reassurance it appears to be, being dominated by dry weeks. \emph{Exposure sensitivity is not a property of a district}: rainfall and temperature displacement share only one district in their top five and are rank-uncorrelated ($\rho=0.24$, $p=0.23$), so no single map identifies the exposure-sensitive districts. The signs are nonetheless geographic rather than noisy --- Colombo loses 7.30\,mm per week under population weighting and Puttalam gains 4.37, as people cluster off and onto the drier coastal strip respectively; 17 districts move down and 9 up.

On the available grid the lapse correction moved exposure about 1.5 times as far as population weighting (0.306 against 0.205\,$^{\circ}$C), reversing the ranking anticipated in the analysis plan. That reversal is coherent: what the correction recovers is exactly what a coarse grid loses, so it grows as the grid coarsens, and it is the one rung that partly repairs the missing $0.1^{\circ}$ product rather than being degraded by it. Largest district shifts were Nuwara Eliya $-1.92\,^{\circ}$C, Badulla $-1.33\,^{\circ}$C and Ratnapura $+1.01\,^{\circ}$C. The B$\to$C step must not be reported as a MAUP effect in full: decomposed, a mean 0.223\,$^{\circ}$C is an orography deficit --- ERA5 issuing temperature for a mountain range about half its true height --- which would apply under area weighting too and belongs to exposure \emph{quality}, while only 0.169\,$^{\circ}$C is genuine sub-grid population placement. The two can oppose each other (Nuwara Eliya, $-2.26$ and $+0.34\,^{\circ}$C), so quoting the combined figure as MAUP would overstate the result by roughly half. Against GHCN-Daily, ERA5 is accurate at sea level and approximately 5\,$^{\circ}$C too warm in the highlands --- an elevation-dependent bias, which is what Build C models: Nuwara Eliya (1{,}880\,m) records 16.40\,$^{\circ}$C against ERA5's 21.17 (bias $+4.77$), while four well-sampled lowland stations sit between $-0.36$ and $-0.42$. A station regression gives a local lapse rate of $6.40\,^{\circ}$C\,km$^{-1}$ and ERA5's own between-cell lapse is $6.31$, both within about 2\% of the $6.5$ applied. Interpolated to Nuwara Eliya's population-weighted elevation the implied true district mean is 19.82\,$^{\circ}$C against Build B's 21.89 (error $+2.07$) and Build C's 19.98 (error $+0.15$), so the correction removes 93\% of the error where it is largest --- bounded by the high end of that regression resting on a single station.

\paragraph{Construction changes alert decisions, and net benefit cannot see it.} At $p^*=0.30$, exposure construction changed the alert decision on 35--76 held-out district-weeks (0.9--1.9\%) for A$'\to$B and 64--104 (1.6--2.6\%) for A$'\to$C, the range spanning four transfer specifications (Table~\ref{tab:wp5-flips}, Fig~\ref{fig:wp5f7}). For scale, the estimand's own contrast --- moving from $\mathcal I^{SC}$ to $\mathcal I^{S}$ --- changed 441 decisions (11.2\%) on the same rows. Exposure construction alone therefore carries 14--24\% of the entire climate block's decision leverage: far from nothing, and far from everything. Under district fixed effects, cluster-bootstrap intervals were 1.94\% [1.27, 2.65] for A$'\to$B and 2.45\% [1.50, 3.57] for A$'\to$C against 11.23\% [9.40, 13.17] for climate removal; at 26 districts and 52 weeks a 1.9\% flip rate is roughly 26 changed alert decisions per year nationally against roughly 152 for the presence or absence of climate information altogether. These estimates are bounded above by a quantity requiring no model: only 6.4\% of held-out rows lie within 0.02 of $p^*$ on the recalibrated scale, so no perturbation of that size can flip more than 6.4\% of decisions however generated, and every estimate is asserted against that bound. The transfer coefficient's norm varies by a factor of 2.3 across specifications ($R^2$ 0.27--0.38, signs stable) but the flip count varies far less, because the flip curve is locally near-linear --- the coefficient would have to be understated several-fold before exposure construction approached the ceiling. The refit below supplies the check the envelope cannot perform on itself, and the envelope survives it.

\emph{A near-zero change in net benefit is not evidence of decision insensitivity.} Across every rung, specification and threshold examined, $|\Delta\mathrm{NB}|$ remained within $\pm0.0023$ while the flip counts above were unambiguously non-zero. This is not flips cancelling by direction --- under this estimate they are strongly asymmetric (A$'\to$B at $p^*=0.30$ added 67 alerts and removed 9), and the refit below finds the same rung near-symmetric with $\Delta\mathrm{NB}$ near zero either way --- but structural, and it follows from the definition of $p^*$ in Section~\ref{sec:framework}. A row can flip only if it sits near $p^*$; rows near $p^*$ have an event rate near $p^*$; and $p^*$ is by construction the break-even rate at which a true positive and its corresponding false positives are worth the same. Every flip is therefore worth approximately zero net benefit in either direction. Measured, the event rate among flipped rows tracks $p^*$ across both rungs and all four thresholds (0.088, 0.160, 0.289, 0.431 against 0.10, 0.20, 0.30, 0.40; $r=0.96$), and the arithmetic closes exactly. The implication reaches past this study: \emph{net benefit is structurally insensitive to any perturbation acting near the decision threshold}, so a near-zero $\Delta\mathrm{NB}$ must not be read as evidence that a perturbation does not affect decisions --- including in the sensitivity analyses reported elsewhere in this paper. Flip counts are accordingly reported alongside $\Delta\mathrm{NB}$ throughout this subsection. Finally, flips require a displacement \emph{and} a prediction near the threshold, and across the 26 districts the flip count correlates with the induced prediction shift but not with exposure displacement alone --- so naming ``exposure-sensitive districts'' from an exposure contrast names the wrong set for decisions as well.

\paragraph{Refitting under each build: the decision moves, the score does not.}\REBUILD{} Refitting M5 on each build's exposure table (Table~\ref{tab:wp5-refit}, Fig~\ref{fig:wp5f7b}) separates two questions the envelope cannot. \emph{The score does not move.} Held-out AUC spans 0.0018 across the four builds (0.7649--0.7668); against Build A$'$, $\Delta$AUC is $+0.0015$ [$-0.0008$, $+0.0037$] under population weighting, $+0.0005$ [$-0.0018$, $+0.0028$] under the orography correction alone and $-0.0003$ [$-0.0040$, $+0.0036$] under both, and $\Delta\mathrm{NB}$ at $p^*=0.30$ is $+0.0013$, $-0.0025$ and $+0.0004$ on intervals of comparable width. Every interval spans zero. \emph{The decision moves.} On the same fits, 72 alert decisions change under population weighting (1.8\%), 64 under the orography correction (1.6\%) and 121 under both (3.1\%), against a swap-path null of 3. Deleting the climate block from the same refit --- the estimand's own $\mathcal I^{SC}\to\mathcal I^{S}$ contrast --- changes 427 decisions (10.9\%) on those rows, so exposure construction carries 15--28\% of the climate block's decision leverage, an independent refit reproducing the 14--24\% the envelope gave and extending its upper end. \emph{A sensitivity analysis reading only $\Delta$AUC and $\Delta\mathrm{NB}$ would have reported a null here}, which is the same structural blindness established above, now shown on a refit rather than on perturbed frozen predictions. The two rungs act differently, which is why the ladder needs a fourth arm: population weighting flips near-symmetrically (35 on, 37 off) while the orography correction is one-signed (15 on, 49 off), so reporting Build C as a single MAUP rung would average a redistribution with a physical correction and describe neither. The one quantity that does move is calibration, and only in M5: its slope rises from 1.085 to 1.108, 1.108 and 1.119, changes of $+0.023$ [$+0.005$, $+0.042$], $+0.024$ [$+0.010$, $+0.037$] and $+0.035$ [$+0.010$, $+0.062$], all excluding zero, while in the reduced specification the same changes span zero and reverse sign. The movement is small and \emph{away} from 1 --- detectable, not clinically material, and not to be headlined --- but it is the only score-level quantity in the analysis whose interval excludes zero, and the null on $\Delta$AUC would have missed it. Three checks bound the refit itself: holding the Build A$'$ knots fixed across arms gives 74, 65 and 119 flips against 72, 64 and 121; the envelope's estimates were 76 against 72 measured for A$'\to$B and 104 against 121 for A$'\to$C, under-counting the second in the direction its own lag-attenuation caveat predicted; and the exact model-free ceiling is never approached, the largest flip rate at any rung or threshold being 3.44\% against a bound of 6.4\%.

One recorded structural argument is corrected by this refit, and the correction generalises. Because M5 carries district fixed effects and the B$\to$C offset is constant within a district (within-district standard deviation $1.5\times10^{-6}\,^{\circ}$C), it was argued that the fixed effects must absorb Build C exactly, so it could flip nothing. The premise holds and the conclusion does not: B$\to$C flips 67 decisions. An additive intercept absorbs a shift in the \emph{linear predictor}, but Build C shifts \emph{temperature}, which enters through a nonlinear distributed-lag cross-basis, and a nonlinear function of a shifted input is not a shifted function of the input. Refitting with temperature entered linearly collapses the flips to 10, which is the measurement that separates the two cases. \emph{A model linear in temperature absorbs a uniform exposure offset; a model with a nonlinear exposure--response does not} --- and the model in use is the second kind. Per district the changed decisions concentrate in the highlands (Nuwara Eliya 7.9\% of its held-out weeks, Matale 7.3\%, Badulla 6.6\%, against 3.1\% nationally) and track the magnitude of the district's Build C offset ($r=0.66$), recovering through a refit the concentration the frozen-prediction screen found; it is a concentration and not an attribution, since Matale ranks second on flips with only the seventh-largest offset.

\paragraph{The result is not an artifact of the population product.} Rebuilding the weight field under WorldPop R2025A and GHS-POP R2023A moved 2.0\% and 1.6\% of a district's weight respectively, against 27.5\% for the area-to-population step itself (Table~\ref{tab:wp5-product}). On exposure the separation is an order of magnitude or more: the product swap moved weekly mean temperature by 0.006--0.009\,$^{\circ}$C against 0.205 for construction, and rainfall by 0.16--0.20\,mm against 3.40. Ratios of construction effect to product effect run from 16.7 to 32.1; they do not narrow in the tails where alerts fire (19.5 in the wettest percentile, 35.8 in the hottest); and in 0 of 26 districts does the product effect reach the weighting effect. Rank correlations of 0.985 and 0.990 on the ranking of movers, with identical top-five sets and complete sign agreement, mean the per-district statements above are not statements about WorldPop. Two incidental findings are worth recording: version drift within one producer exceeded the gap between producers (WorldPop UN-adjusted versus WorldPop's own R2025A, mean total variation 0.0201, against 0.0157 between WorldPop and GHS-POP), so ``same producer, newer release'' is not the safer substitution it appears; and the $\approx$1.4\% of population falling outside the 26 polygons under WorldPop UN-adjusted is a coastline artifact specific to that product and vintage (GHS-POP loses 0.23\%, WorldPop R2025A 0.10\%), to be reported as a bounded product sensitivity rather than a property of the geometry. All three products are modelled surfaces sharing much of their input census, so their agreement bounds method sensitivity, not the truth of where people live; and whether product choice moves $\Delta V_{C,k}$ is not measured, because the refit varies the \emph{construction} and not the product. Since the product effect on exposure is 16--32 times smaller than the construction effect, and the construction effect moves neither $\Delta$AUC nor $\Delta\mathrm{NB}$ detectably, a detectable product effect on either is implausible --- an inference from the exposure contrast, not a measurement, and not reported as a null.

\paragraph{Where miscalibration and decision changes concentrate.} Six of 60 pre-specified associations survived false-discovery control (Fig~\ref{fig:wp5f8}), and the largest is a caution rather than a result. The strongest association in the entire screen was between a unit's calibration intercept and its own alert prevalence ($\rho=0.85$, $q<0.001$), which is close to tautological --- a unit's intercept absorbs its base rate --- and is reported because it outranked every geospatial modifier by a wide margin, demonstrating what the screen would otherwise have hidden: any modifier correlated with outbreak burden inherits this association. Per-unit calibration slope ranged from 0.67 to 2.23, so the model is not uniformly calibrated in space, and was positively associated with cropland fraction ($\rho=0.61$, $q=0.031$) and land-cover diversity ($\rho=0.59$, $q=0.032$) and negatively with population density ($\rho=-0.50$); all three survive adjustment for the unit's alert prevalence (partial $\rho=0.50$, $p=0.010$; $0.49$, $p=0.011$; $-0.44$, $p=0.025$), so they are not the tautology in disguise. Predictions are too widely spread in dense built-up units and too compressed in agricultural, land-cover-heterogeneous ones. Exposure-driven flips were associated with slope ($\rho=0.56$, $q=0.035$), elevation ($\rho=0.56$, $q=0.035$) and height-above-nearest-drainage ($\rho=0.55$, $q=0.038$), with Badulla (9.9\% of weeks), Nuwara Eliya (9.3\%) and Ratnapura (6.0\%) carrying a large share --- coherent with those being where the lapse correction moves exposure most, but elevation and the measured B$\to$C displacement are strongly collinear across 26 units ($\rho=0.74$) and neither survives adjustment for the other, so the concentration is real while its attribution is not identified at this sample size, and we do not assign it. No modifier reached significance for per-unit $\Delta\mathrm{NB}$ (strongest, population density, $\rho=0.40$, $q=0.15$); given the structural argument above this is the expected result rather than an additional null, and it is the fourth independent appearance of the same mechanism --- after the national envelope, the refit and the threshold sweep.

\paragraph{Spatially-honest cross-validation is expensive, and residual autocorrelation does not price it.} Sri Lanka is approximately 430\,km end to end, so a buffer is necessarily a large fraction of the country (Table~\ref{tab:wp4-power}): excluding only adjacent districts leaves a median of 21 of 25 possible training districts (worst fold 16) and retains 84\% of training district-weeks, while at 100\,km the median fold trains on 9 and the worst on 5, retaining 36\%, and at 150\,km the scheme collapses with 12 of 26 folds keeping fewer than 5. Across every radius and fold, zero train--test pairs fell within the buffer. Moran's $I$ on residuals was indistinguishable from its permutation null at every distance band for all three models examined (permutation $p$ 0.53--0.99) and the variogram was flat from 25 to 400\,km; this is more robust than it first appears, because the full model's district fixed effects could manufacture a null but the geomatics-only model carries none and returns the same answer ($I=0.001$ among adjacent districts, $p=0.99$) --- though with 26 units the permutation null has a standard deviation of about 0.02, so this excludes residual spatial correlation above roughly $|I|=0.04$ rather than proving its absence.

Direct measurement contradicts the inference that would follow (Table~\ref{tab:wp4-fold}, Fig~\ref{fig:wp4f}). Against a temporal-split AUC of 0.601 and an unbuffered leave-one-out AUC of 0.592, adjacency buffering scored 0.565 while a random-district control trained on the same 21 districts scored 0.587 --- a geography-attributable gap of $-0.022$. The gap widens with radius: at 100\,km the spatially blocked estimate is 0.502, indistinguishable from chance, while an equally sized random training set still reaches 0.573. Because training-set size is held fixed by construction, that difference cannot be sample size, and in 0 of 10 control draws at 0, 75 and 100\,km did a random subset score below the buffered fold. Calibration degrades along the same gradient, the slope falling from 0.875 under the temporal split to 0.529 at adjacency buffering and 0.018 at 100\,km, and the effect is not one district's doing: 17 of 26 districts lose discrimination under adjacency buffering and 9 gain. Power is the binding constraint on certifying this. Cluster-bootstrap intervals for the buffered-minus-control gap span zero at 0\,km ($-0.022$ [$-0.055$, $+0.011$]) and exclude it only at 75\,km ($-0.061$ [$-0.119$, $-0.008$]) and 100\,km ($-0.070$ [$-0.143$, $-0.001$]); the point estimates are consistent and one-signed at every radius, but with 26 clusters the bootstrap has little to work with. \textbf{Intervals spanning zero here mean no difference was detected, not that none exists, and with no better-powered spatial companion in this study the result stands as a characterised limitation rather than a certified claim.}

On M5 the answer is different, and at the operative radius it has the opposite sign.\REBUILD{} Refitting M5 under the same folds (Table~\ref{tab:wp4-fold-m5}, Fig~\ref{fig:wp4fm5}) gives a buffered-minus-control gap of $+0.038$ at adjacency buffering, $+0.024$ at 25\,km, $+0.003$ at 50\,km, $-0.054$ at 75\,km and $-0.027$ at 100\,km: where the geomatics-only model \emph{loses} 0.022 to geography at the operative radius, M5 \emph{gains} 0.038. The two disagree in sign at 0\,km and agree only at 100\,km, so the price of a fold is a property of the model as much as of the geography and the within-model result on one model cannot substitute for the other. The dependence on radius is not monotone either --- the gap turns at 75\,km and recovers at 100 --- so the reading that the cost grows steadily with radius, available from the geomatics-only model alone, is withdrawn for this model. Nor is the effect monotone in training-set size: adjacency buffering scores 0.743 against unbuffered leave-one-out's 0.720 while training on four fewer districts, an inversion we cannot explain and do not smooth over; it is the reason the estimand is the matched-control gap, which holds size fixed by construction. Certification rests on the cluster bootstrap rather than the control draws, and the two disagree: at 0\,km all 10 controls score below the buffered fold ($z=+2.57$) while the bootstrap interval spans zero ($+0.038$ [$-0.012$, $+0.111$]). Exactly one radius survives, 75\,km, at $-0.054$ [$-0.092$, $-0.016$] on AUC and $-0.030$ [$-0.049$, $-0.013$] on $\Delta\mathrm{NB}$ at $p^*=0.30$. Most consequentially for this paper, fitting $\text{M5}_{\text{no-climate}}$ under every fold answers the concern this analysis exists to address: the climate block's contribution to net benefit is $+0.017$ under the temporal split and $+0.009$, $+0.015$, $+0.015$, $+0.009$ and $+0.016$ under buffered folds at 0--100\,km, every change-versus-temporal interval spanning zero and every point within 0.008 of the temporal value. \emph{Spatial cross-validation does not deflate $\Delta V_{C,k}$.} The buffered estimates must not be read as a gain over the temporal split --- the intervals do not support that --- and the honest reading is that the increment is unchanged. The radius dependence localises in the climate block (the twin's gap stays within $[+0.017,-0.015]$ across radius while M5's falls to $-0.054$), which is coherent with climate fields being smooth over hundreds of kilometres where case history is not; and the national number hides district-level disagreement, the pooled gap at 0\,km being $+0.038$ while 15 of 26 districts are individually negative.

The methodological point nonetheless generalises: \emph{residual spatial autocorrelation and dependence on spatially proximate training data are different quantities, and they can disagree}. Moran's $I$ on residuals asks whether what remains after fitting on every unit is spatially clustered; a buffered fold asks whether a model can generalise to a region it has never seen. A model can pass the first and fail the second, and the model examined here does --- so selecting a buffer radius from a residual-range analysis, the conventional design and the one this study's own plan specified, cannot be relied on to price the folds. Two limits bound what this establishes. M5 was refitted here on a rebuilt input rather than on the registered one\,\REBUILD, so the cross-model result is a sensitivity analysis: it answers the concern this analysis exists to address in the favourable direction, and the registered re-run is what would settle it. And it says nothing about absolute performance: the geomatics-only model's outcome labels are a reconstruction carrying an exploratory-target caveat on 19.2\% of test rows, identical across arms so it cannot manufacture a between-arm difference, but sufficient to bar quoting any single AUC here as that model's performance.

\begin{table}[htbp]
\centering
\caption{\textbf{The exposure ladder: how far each construction step moves exposure.} Mean absolute displacement over 10{,}842 district-weeks (26 RDHS $\times$ 417 ISO weeks, 2018--2025). A$\to$A$'$ exchanges an \texttt{all\_touched} mask for fractional-area weighting; A$'\to$B re-weights area to population; B$\to$C corrects temperature to the population's elevation. Temperature figures are conservative floors (Section~\ref{sec:methods-wp5}).}
...
\begin{table}[htbp]
\centering
\caption{\textbf{Alert decisions changed by exposure construction, $p^*=0.30$.} Held-out panel of 3{,}926 district-weeks (26 RDHS $\times$ 151 weeks, 2023--2025). Ranges span four transfer specifications; intervals are 95\% percentile cluster bootstrap (2{,}000 replicates, districts resampled) under the district-fixed-effects specification. The climate-removal row is the estimand's own contrast, $\mathcal I^{SC}\to\mathcal I^{S}$.}
\label{tab:wp5-flips}
...
50\,km  & 15 & 0.558 & 0.577 & $-0.019$ [$-0.054$, $+0.018$] & 2 of 10 \\
75\,km  & 12 & 0.516 & 0.578 & $-0.061$ [$-0.119$, $-0.008$] & 0 of 10 \\
100\,km & 9  & 0.502 & 0.573 & $-0.070$ [$-0.143$, $-0.001$] & 0 of 10 \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{\textbf{Refitting M5 under each build: the decision moves, the score does not.}\REBUILD{} Same held-out panel of 3{,}926 district-weeks; the model is refitted on each build's exposure table with cross-basis knots re-derived per arm. Deltas are against Build A$'$; intervals are 95\% percentile cluster bootstrap (2{,}000 paired replicates over the 26 RDHS units). Flips are held-out alert decisions changing status at $p^*=0.30$. The swap-path null rebuilds Build A$'$ through the identical machinery and refits, giving the flip count the pipeline produces when nothing changes; the last row is the estimand's own $\mathcal I^{SC}\to\mathcal I^{S}$ contrast computed on the same refit.}
\label{tab:wp5-refit}
{\footnotesize\begin{tabular}{@{}lrlrlr@{}}
\toprule
...
$\mathcal I^{SC}\to\mathcal I^{S}$ & 0.7323 & $-0.0330$                 & 1.156 & $+0.071$                      & 427 (10.9) \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{\textbf{The same fold cost, measured on M5.}\REBUILD{} Design identical to Table~\ref{tab:wp4-fold}: every buffered fold matched to a random-district control trained on the same number of districts (10 draws). Reference points: temporal split AUC 0.765, unbuffered leave-one-out 0.720. Intervals are 95\% percentile cluster bootstrap. At adjacency buffering the gap has the \emph{opposite sign} to the geomatics-only model; the ``controls below'' column and the bootstrap interval disagree there, and the interval is what the claim rests on.}
...
\safeincludegraphics[width=\linewidth]{submission_figs/WP5_F2_exposure_contrast.pdf}
\caption{\textbf{Exposure construction contrast across the three-build ladder, Sri Lanka 2018--2025.} Displacement in weekly district exposure attributable to the construction of the exposure variable alone, with weather fields, geometry and temporal alignment held fixed. Panels show per-district displacement for rainfall and temperature under population weighting (A$'\to$B) and the additional lapse correction (B$\to$C), the decomposition of the mask step from the weighting step, the behaviour of the contrast in the wet tail where alerts fire, and the spatial pattern of each. Rainfall and temperature displacement are effectively uncorrelated across districts ($\rho=0.24$, $p=0.23$), so no single map identifies the exposure-sensitive districts. Maps are drawn in EPSG:32644; the three map panels use a diverging ramp symmetric in luminance, so the largest movers are labelled numerically on the map face for greyscale reproduction.}
\label{fig:wp5f2}
\end{figure}

\begin{figure}[htbp]\centering
\safeincludegraphics[width=\linewidth]{submission_figs/WP5_F7_decision_flip_envelope.pdf}
\caption{\textbf{Decision-flip envelope: exposure construction changes alerts that net benefit cannot see.} Held-out panel of 3{,}926 district-weeks, 2023--2025. The exact flip curve gives the maximum share of alert decisions any perturbation of a given size can change, as a property of the frozen predictions alone; estimated flip counts for each exposure rung are bounded by it, and the $\mathcal I^{SC}\to\mathcal I^{S}$ contrast provides the observed ceiling. Lower panels show why $\Delta\mathrm{NB}$ stays within $\pm0.0023$ while flip counts are clearly non-zero: rows that flip lie near $p^*$, and rows near $p^*$ have an event rate near it, which is by definition the break-even rate at which a flip is worth approximately zero net benefit in either direction.}
\label{fig:wp5f7}
\end{figure}

\begin{figure}[htbp]\centering
\safeincludegraphics[width=\linewidth]{submission_figs/WP5_F7b_decision_sensitivity_refit.pdf}
\caption{\textbf{Refitting under each build separates what the model scores from what it decides.}\REBUILD{} The model is refitted on each build's exposure table and evaluated on the same 3{,}926 held-out district-weeks; deltas are against Build A$'$, with 95\% cluster-bootstrap intervals over the 26 units. (a) Neither discrimination nor net benefit moves: every $\Delta$AUC and $\Delta\mathrm{NB}$ interval spans zero under every rung and both specifications. (b) Calibration slope does move, and only in M5 (filled): all three intervals exclude zero, while the specification without district fixed effects (open) spans zero and reverses sign. (c) Alert flips against the threshold, with the swap-path noise floor shaded --- rebuilding Build A$'$ through the identical machinery changes at most 3 decisions; open stars mark the envelope estimates at $p^*=0.30$, which land on the refit for population weighting and below it for full Build C. (d) The two rungs act differently: population weighting is near-symmetric in direction, the orography correction one-signed. (e) Why a district fixed effect does not absorb the Build C offset even though the offset is constant within district: entering temperature linearly leaves 10 flips, entering it through the nonlinear cross-basis leaves 67, in models that both carry the fixed effects. (f) Changed decisions track the magnitude of a district's Build C temperature offset ($r=0.66$), concentrating in the highlands without being determined by displacement alone.}
\label{fig:wp5f7b}
\end{figure}

\begin{figure}[htbp]\centering
\safeincludegraphics[width=\linewidth]{submission_figs/WP5_F8_miscalibration_structure.pdf}
\caption{\textbf{Where miscalibration and exposure-driven decision changes fall in space.} Per-unit calibration and decision quantities from the frozen held-out predictions, screened against 15 geospatial modifiers fixed in advance. (a) Exposure-driven alert changes concentrate in three highland units. (b) Calibration slope varies from 0.67 to 2.23, so the model is not uniformly calibrated in space. (c) Flips track altitude, but elevation and measured exposure displacement are collinear across 26 units and neither survives adjustment for the other, so the concentration is reported without attribution. (d) Calibration slope tracks cropland fraction, surviving adjustment for the unit's own alert prevalence. (e) All 60 tests, filled where they survive Benjamini--Hochberg control at $q<0.05$, so the figure cannot be read as a selected subset. (f) No modifier explains per-unit $\Delta\mathrm{NB}$. The strongest association in the screen is between calibration intercept and alert prevalence, which is near-tautological and reported as a caution: any modifier correlated with outbreak burden inherits it. Exploratory-explanatory; no modifier here may re-enter the model as an accuracy predictor.}
\label{fig:wp5f8}
\end{figure}

\begin{figure}[htbp]\centering
\safeincludegraphics[width=\linewidth]{submission_figs/WP4_F_fold_effect_within_m6.pdf}
\caption{\textbf{What spatially buffered folds cost, measured against a size-matched control.} Discrimination and calibration under buffered leave-one-out cross-validation at increasing buffer radius, each buffered fold matched to a random-district control trained on an identical number of districts so the loss of training data is held fixed and the remaining gap is attributable to geography. Residual spatial autocorrelation is undetectable in these models at every distance band, yet performance still depends on having geographically proximate districts in training --- the two quantities disagree, so a residual-range analysis cannot be used to price the folds.}
\label{fig:wp4f}
\end{figure}

\begin{figure}[htbp]\centering
\safeincludegraphics[width=\linewidth]{submission_figs/WP4_F_fold_effect_within_m5.pdf}
\caption{\textbf{The same measurement on M5: the price of a fold depends on the model.}\REBUILD{} Design identical to Fig~\ref{fig:wp4f}, applied to M5 refitted on the rebuilt input. (a) Performance under each scheme, with the temporal split and unbuffered leave-one-out as reference lines; the buffered arm is \emph{above} the size-matched control at short radii and below it at 75\,km. (b) The geography-attributable gap with cluster-bootstrap intervals: not monotone in radius, excluding zero at 75\,km only, and opposite in sign at adjacency buffering to the same panel in Fig~\ref{fig:wp4f}. (c) Training districts remaining, the power cost the matched control removes from the comparison. (d) The climate block's contribution to net benefit survives buffering at every radius and is statistically unchanged from the temporal split. (e) The radius dependence localises in the climate block: the climate-free twin stays flat while M5 falls away. (f) The national number hides district-level disagreement: at 100\,km the gap is negative in 20 of 26 districts.}
\label{fig:wp4fm5}
\end{figure}
...
Framed through the matched comparison of Section~\ref{sec:framework}, the estimand is the metric-specific increment $\Delta V_{C,k}=V_k(\mathcal I^{SC})-V_k(\mathcal I^{S})$, and the results say it is small and not shown to be robust across metrics once the comparator carries matched non-climate structure. The guiding distinction is
\[
\text{biological relevance}\ \neq\ \text{incremental predictive value}\ \neq\ \text{incremental decision value}:
\]
a climate variable can be biologically meaningful, and even individually predictive, yet add little short-lead decision value once recent cases are observed --- not a contradiction, but a statement about how much of the climate signal the surveillance information set already carries. Two natural extensions are defined but not estimated here and belong to a separate, pending workstream: an incremental \emph{geomatics} estimand $\Delta V_{G}$ (a standalone geomatics model estimates $V(\mathcal I^{G})$, which is \emph{not} $V(\mathcal I^{SG})-V(\mathcal I^{S})$, so the present study makes no geomatics-value claim), and a population-weighted or fractional spatial-exposure operator $\mathcal A_w[X]=\int_{D_i}wX/\int_{D_i}w$ (only the area mean, $w{\equiv}1$, was used; alternative weightings are a change-of-support sensitivity, not a forecasting result).
...
\subsection*{Exposure construction and spatially-honest validation}
Two Sri Lanka analyses (Section~\ref{sec:wp45}) bear on how far the deflationary reading should be pushed. The first is a caution about the metric rather than about climate: net benefit is structurally insensitive to any perturbation acting near the decision threshold, because a row can only flip if it sits near $p^*$ and rows near $p^*$ are by construction break-even. Exposure construction changed 0.9--3.1\% of alert decisions while $|\Delta\mathrm{NB}|$ stayed within $\pm0.0025$, and the same mechanism appeared four independent times in that analysis --- including on a refit, where 121 alert decisions changed sign while $\Delta\mathrm{NB}$ moved $+0.0004$ [$-0.0022$, $+0.0031$], so it is not an artifact of holding the model fixed. This qualifies sensitivity analyses in this literature that read a near-zero $\Delta\mathrm{NB}$ as ``no decision effect'' --- including analyses reported elsewhere in this paper --- and the concrete recommendation is to report a decision-flip count alongside $\Delta\mathrm{NB}$ whenever the perturbation acts near the threshold. The second is that how areal exposure is built is an unreported analyst degree of freedom carrying 15--28\% of the entire climate block's decision leverage, with the weighting choice mattering roughly four times more than the masking choice --- and it is the weighting that studies leave unstated. Neither analysis revises $\Delta V_{C,k}$ itself, and the refit that translates them into $\Delta$AUC, $\Delta$calibration and $\Delta V_{C,k}$ measures no detectable movement in any of the three: across four exposure builds every interval spans zero, on a refit run over an independently rebuilt input\,\REBUILD{} rather than the registered one. What moves is the decision and, marginally, calibration. The registered re-run remains outstanding.

The spatial cross-validation carries a methodological result and a limitation of comparable size. Residual spatial autocorrelation and dependence on spatially proximate training data are different quantities and can disagree: residuals here look spatially clean at every distance band while discrimination still falls materially when neighbouring districts are withheld, so choosing a buffer radius from a residual-range analysis --- the conventional design --- cannot price the folds. The limitation is power. At $n=26$ the buffered-minus-control gap is one-signed at every radius but its interval spans zero at the operative radius, and this study reports no better-powered spatial companion, so the finding stands as a characterised limitation rather than a certified claim. In particular it does not establish that the headline comparison survives spatial cross-validation, which was not refitted under the folds; if the hybrid depends on spatial proximity as the geomatics-only model does, spatially honest validation would move it too.
...
\bibitem{Openshaw1984} Openshaw S. The Modifiable Areal Unit Problem. Concepts and Techniques in Modern Geography, No.\,38. Norwich, UK: Geo Books; 1984.

\bibitem{Fotheringham1991} Fotheringham AS, Wong DWS. The modifiable areal unit problem in multivariate statistical analysis. Environ Plan A. 1991;23(7):1025--1044. doi:\,10.1068/a231025.

\bibitem{Roberts2017} Roberts DR, Bahn V, Ciuti S, Boyce MS, Elith J, Guillera-Arroita G, et al. Cross-validation strategies for data with temporal, spatial, hierarchical, or phylogenetic structure. Ecography. 2017;40(8):913--929. doi:\,10.1111/ecog.02881.

\bibitem{Valavi2019} Valavi R, Elith J, Lahoz-Monfort JJ, Guillera-Arroita G. blockCV: an R package for generating spatially or environmentally separated folds for k-fold cross-validation of species distribution models. Methods Ecol Evol. 2019;10(2):225--232. doi:\,10.1111/2041-210X.13107.

\bibitem{Hersbach2020} Hersbach H, Bell B, Berrisford P, Hirahara S, Hor\'anyi A, Mu\~noz-Sabater J, et al. The ERA5 global reanalysis. Q J R Meteorol Soc. 2020;146(730):1999--2049. doi:\,10.1002/qj.3803.

\bibitem{Carver2023ARCO} Carver RW, Merose A. ARCO-ERA5: An analysis-ready cloud-optimized reanalysis dataset. 22nd Conference on Artificial Intelligence for Environmental Science, American Meteorological Society; 2023.

\bibitem{AlduchovEskridge1996} Alduchov OA, Eskridge RE. Improved Magnus form approximation of saturation vapor pressure. J Appl Meteorol. 1996;35(4):601--609. doi:\,10.1175/1520-0450(1996)035<0601:IMFAOS>2.0.CO;2.

\bibitem{CopernicusDEM2021} European Space Agency, Sinergise. Copernicus Global Digital Elevation Model (GLO-30). Distributed by OpenTopography; 2021. doi:\,10.5069/G9028PQB.

\bibitem{Menne2012} Menne MJ, Durre I, Vose RS, Gleason BE, Houston TG. An overview of the Global Historical Climatology Network-Daily database. J Atmos Ocean Technol. 2012;29(7):897--910. doi:\,10.1175/JTECH-D-11-00103.1.

\bibitem{Schiavina2023} Schiavina M, Freire S, Carioli A, MacManus K. GHS-POP R2023A: GHS population grid multitemporal (1975--2030). European Commission, Joint Research Centre; 2023. doi:\,10.2905/2FF68A52-5B5B-4A22-8F40-C41DA8332CFE.

\bibitem{BenjaminiHochberg1995} Benjamini Y, Hochberg Y. Controlling the false discovery rate: a practical and powerful approach to multiple testing. J R Stat Soc Series B. 1995;57(1):289--300. doi:\,10.1111/j.2517-6161.1995.tb02031.x.
...
\noindent\safeincludegraphics[width=0.8\textwidth]{submission_figs/S1_Fig.pdf}\\
...
\noindent\safeincludegraphics[width=0.9\textwidth]{submission_figs/S2_Fig.pdf}\\
...
\noindent\safeincludegraphics[width=0.55\textwidth]{submission_figs/S3_Fig.pdf}\\
...
\noindent\safeincludegraphics[width=0.9\textwidth]{submission_figs/Fig2.pdf}\\
...
\noindent\safeincludegraphics[width=0.7\textwidth]{submission_figs/S4_Fig.pdf}\\
...
\noindent\safeincludegraphics[width=0.86\textwidth]{submission_figs/Fig1_pipeline.pdf}\\
...
Colombia & full climate & 0.6162 & 0.2127 & 0.109 & 0.725 \\
 & matched no-climate & 0.6251 & 0.2169 & 0.113 & 0.713 \\
